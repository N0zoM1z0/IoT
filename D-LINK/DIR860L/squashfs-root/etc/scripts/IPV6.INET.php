...
function dev_attach($hasevent)
{
	...
	// HuanYao: router MTU and lifetime 
	$mtu = XNODE_get_var($_GLOBALS["INF"]."_MTU");
	XNODE_del_var($_GLOBALS["INF"]."_MTU");
	set("mtu", $mtu);
	...
	{
	$routerlft = XNODE_get_var($_GLOBALS["INF"]."_ROUTERLFT");
	XNODE_del_var($_GLOBALS["INF"]."_ROUTERLFT");
	set("routerlft", $routerlft);
	}
	...
	if($M=="STATEFUL" || $child!="")
	{
		// The devie should pause for one seconds, wait the DAD complete and send confirm messages.
		cmd('event WANPORT.LINKUP insert V6CONFIRM:"sleep 3; kill -SIGUSR1 `cat '.$pidfile.'`"');
