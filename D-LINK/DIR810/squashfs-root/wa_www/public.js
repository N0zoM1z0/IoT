...
function check_key_1()
{
	...
	for(var i = 1; i < 2; i++){
		...
		if (wep_def_key[i-1].selectedIndex){
			...
			if (key.length != wep_key_len && key.length != hex_len){
				alert(which_lang[TEXT041_1]+" " + [i+4] + " "+which_lang[TEXT041_2]+" " + wep_key_len + " "+which_lang[TEXT041_3]+" " + hex_len + " "+which_lang[TEXT041_4]);//TEXT041			    		
				return false;
			}else if(key.length == hex_len){
				for (var j = 0; j < key.length; j++){
					if (!check_hex(key.substring(j, j+1))){
					alert(which_lang[TEXT042_1]+" " + [i+4] + " "+which_lang[TEXT042_2]);//TEXT042			      			
						return false;
					}
				}
				if(i == def_key)
...
function copy_ip(index){
	...
	if (get_by_id("ip_list" + index).selectedIndex > 0){
		...
		alert(which_lang[SELECT_COMPUTER_ERROR]);
	}
}
