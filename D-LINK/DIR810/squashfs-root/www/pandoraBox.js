var noAuthPage = new Array('login.asp', 'login_fail.asp', 'logout.asp', 'es_base.asp', 
		...
		'wizard_router.asp', 'wa_login.asp', 'reject.htm', 'goto_mydlink.asp');
var noRedirectLogin = new Array('tools_firmw.asp', 'login.asp', 'reject.asp', 'time_ctl.asp', 'st_device.asp',
		...
		"wizard_mydlink.asp", 'goto_mydlink.asp','');
var gConfig;
...
function get_router_info() 
{
	...
	var info = {
		...
		'wan_mac':		config_val('wan_mac'),
		'region':		config_val('region'),
		'isRussia':		config_val('isRussia'),
		'ch2_lst':		config_val('ch2list'),
		'ch5_lst':		config_val('ch5list'),
		'ch5_DFS_lst':	config_val('ch5DFSlist'),
		'domain':		config_val('RF_Domain'),
		'WFA':			config_val('WFA_1_00')
	};
...
// 2013-10-09 service name accept '\' 

function check_service_name(name)
{
	var re = /[`;:|'"]/;
	if(re.test(name)){
		return false;
...
** 
*/
function ajax_submit_new(param,isAsync)
{
	var time=new Date().getTime();
	var ajax_param = {
		type: 	"POST",
...
/*
** Silvester
** check ssid ascii character between 32 and 126 
*/
