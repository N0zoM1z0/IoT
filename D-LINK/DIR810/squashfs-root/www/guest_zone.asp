...
<script>
	...
	var lanCfg = {
		...
		'sMode':			config_str_multi("lanWlanCfg_SecurityMode_"),
		'wdsenable':		config_str_multi("lanWlanCfg_WDSEnable_"),
		'macaddr':			config_str_multi("wlanRmMac_MACAddress_"),
		'enableRouting':	config_str_multi("lanWlanCfg_RouteBetweenZone_")
	}
	...
	var wpaCfg = {
		...
		'encrMode':		config_str_multi("wpaInfo_EncryptionMode_"),
    ...
    function check_schedule(i)
    {
		...
		var host_sched_name = (i==0)?lanCfg.schedule[0]:lanCfg.schedule[2];//0 2
		var guest_sched_name = (i==0)?$("#wlan0_vap1_schedule_select").val():$("#wlan1_vap1_schedule_select").val();	//0 1
		var lan_enable = (i==0)?lanCfg.enable[0]:lanCfg.enable[2];//0 2
		var gz_enable = (i==0)?$("#guest_zone_enable0")[0]:$("#guest_zone_enable1")[0];	//0 1
...
//        }
    ...
    function send_request()
    {
        ...
        if (!(check_ssid_0("show0_ssid") && ischeck_wps("auto")))
                return false;
        if (!(check_ssid_0("show1_ssid") && ischeck_wps("auto")))
                return false;

        if(!check_ascii($('#show0_ssid').val()) || !check_ascii($('#show1_ssid').val())){
	...
	**	Date:	2013-06-07
	**	Author:	Silvia Chang
	**	Reason:	Fixed wep-share should not pop WPS disable message for wireless guest zone
	...
	**/
	...
	function submit_All()
	{
		...
		var paramForm = {
			url: "get_set.ccp",
			arg: 'ccp_act=set&ccpSubEvent=CCP_SUB_WIRELESS&nextPage=guest_zone.asp'
		};

		for(var i=0; i<2;)
		{
			paramForm.arg += "&lanWlanCfg_Enable_1.1."+ins+".0.0="+get_checked_value($('#guest_zone_enable'+i)[0]);
			paramForm.arg += "&lanWlanCfg_ScheduleIndex_1.1."+ins+".0.0="+$('#wlan'+i+'_vap1_schedule_select').val();
			paramForm.arg += "&lanWlanCfg_SSID_1.1."+ins+".0.0="+urlencode($('#show'+i+'_ssid').val());
			paramForm.arg += "&lanWlanCfg_SecurityMode_1.1."+ins+".0.0="+$('#wep'+i+'_type')[0].selectedIndex;
			paramForm.arg += "&lanWlanCfg_RouteBetweenZone_1.1."+(ins-1)+".0.0="+get_checked_value($('#guest_select_'+i)[0]);
			paramForm.arg += "&lanWlanCfg_RouteBetweenZone_1.1."+ins+".0.0="+get_checked_value($('#guest_select_'+i)[0]);
			i++;
			ins+=2;
		}
	...
	function Format_WEP(j)
	{
		...
		for(var i=1; i<=4; i++)
		{
			WEP += "&wepKey_KeyHEX64_1.1."+ins+".1."+i+"="+ $('#wlan'+j+'_vap1_wep64_key_'+i).val();
			WEP += "&wepKey_KeyHEX128_1.1."+ins+".1."+i+"="+ $('#wlan'+j+'_vap1_wep128_key_'+i).val();
		}
		submit_c += WEP;
	}
	...
	function Format_WPA(j)
	{
		...
		WPA += "&wpaInfo_KeyUpdateInterval_1.1."+ins+".1.0="+ $('#wlan'+j+'_vap1_gkey_rekey_time').val();
		WPA += "&wpaInfo_AuthenticationTimeout_1.1."+ins+".1.0="+$('#wlan'+j+'_vap1_eap_reauth_period').val();
		WPA += "&wpaPSK_KeyPassphrase_1.1."+ins+".1.1="+ urlencode($('#wlan'+j+'_vap1_psk_pass_phrase').val());
		...
		<td valign="top" width="150" id="sidehelp_container" align="left">
			...
			<tr>
				<td id=help_text><strong><script>show_words(_hints)</script>&hellip;</strong>
					...
					<p><a href="support_adv.asp#GuestZone" onclick="return jump_if();">
