...
*/
		...
		paramSet = {
			url: "get_set.ccp",
			arg: "ccp_act=setStorage&ccpSubEvent=CCP_SUB_WEBACCESS&nextPage=storage.asp"
		};
		...
		*/
		...
		for(var k=0; k<usrName.length; k++)
		{
			...
			if(edit_idx != "")
				submitUserData += "&igdStorageUser_Username_1.1."+(edit_idx)+".0.0="+usrName[k].split("/")[0];
		}
		...
		for(var i=0; i<DataArray.length; i++)
		{
			if(DataArray[i].Name == "admin") //admin rule
			{
				...
				{
					submitData += '&igdStorageAdminRule_Device_1.1.1.'+(admin_idx)+'.0='+DataArray[i].Device; 
					submitData += '&igdStorageAdminRule_AccessPath_1.1.1.'+(admin_idx)+'.0='+encodeURIComponent(DataArray[i].Access_path);
					submitData += '&igdStorageAdminRule_Permission_1.1.1.'+(admin_idx)+'.0='+DataArray[i].Permission; 
					admin_idx++;
				}
			}
			else if(DataArray[i].Name == "guest") //guest rule
			{
				if(DataArray[i].Access_path !="None")
				{
					submitData += '&igdStorageGuestRule_Device_1.1.1.'+(guest_idx)+'.0='+DataArray[i].Device;
					submitData += '&igdStorageGuestRule_AccessPath_1.1.1.'+(guest_idx)+'.0='+encodeURIComponent(DataArray[i].Access_path);
					submitData += '&igdStorageGuestRule_Permission_1.1.1.'+(guest_idx)+'.0='+DataArray[i].Permission;
					//clear the rest of rules
					for(var k=(guest_idx+1); k<=guest_rule_inst.length; k++)
					{
						submitData += '&igdStorageGuestRule_Device_1.1.1.'+k+'.0=0';
						submitData += '&igdStorageGuestRule_AccessPath_1.1.1.'+k+'.0=';
						submitData += '&igdStorageGuestRule_Permission_1.1.1.'+k+'.0=0';
					}
					guest_idx++;
				}
				else
			...
			else //user
			{
				if(DataArray[i].Num!="")
				{
					...
					for(var j=0; j<usrPasswd.length; j++) //save username and password
					{
						if(DataArray[i].Name == usrPasswd[j].split("/")[0])
						{
							...
							{
							submitData += "&igdStorageUser_Username_1.1."+(user_idx)+".0.0="+DataArray[i].Name;
				...
				{					
					submitData += '&igdStorageUserRule_Device_1.1.'+(user_idx-1)+'.'+rule_user_idx+'.0='+DataArray[i].Device;
					submitData += '&igdStorageUserRule_AccessPath_1.1.'+(user_idx-1)+'.'+rule_user_idx+'.0='+encodeURIComponent(DataArray[i].Access_path);
					submitData += '&igdStorageUserRule_Permission_1.1.'+(user_idx-1)+'.'+rule_user_idx+'.0='+DataArray[i].Permission;
					//clear the rest of rules
					for(var k=(rule_user_idx+1); k<=5; k++)
					{
						submitData += '&igdStorageUserRule_Device_1.1.'+(user_idx-1)+'.'+k+'.0=0';
						submitData += '&igdStorageUserRule_AccessPath_1.1.'+(user_idx-1)+'.'+k+'.0=';
						submitData += '&igdStorageUserRule_Permission_1.1.'+(user_idx-1)+'.'+k+'.0=0';
					}
					rule_user_idx++;
				}
				else
...
*/
		...
		for(var k=user_idx; k<=(user_rule_inst.length / ary_user_name.length); k++)
			{
				submitData += '&igdStorageUser_Username_1.1.'+k+'.0.0=';
				submitData += '&igdStorageUser_Password_1.1.'+k+'.0.0=';
				
				for(var m=1; m<=5; m++)
				{
					submitData += '&igdStorageUserRule_Device_1.1.'+k+'.'+m+'.0=0';
					submitData += '&igdStorageUserRule_AccessPath_1.1.'+k+'.'+m+'.0=';
					submitData += '&igdStorageUserRule_Permission_1.1.'+k+'.'+m+'.0=0';
				}
			}
	...
	function DataShow()
	{
		...
		var contain = "";
			...
			contain +='<td colspan="6"><img src="modify.png" width="20px"> : '+get_words("_modify")+'&nbsp;&nbsp;<img src="remove.png" width="20px"> :'+get_words("_delete")+'</td>';
			contain +='</tr>';
			contain +='<tr>';
		...
		{	
			...
			if(DataArray[i].Name != "")
			{
				...
				{
					if(DataArray[i].Access_path == "None")
						icon += "<td align=\"center\" width=\"5%\"><img src=\"modify.png\" id=\"modify\" width=\"20px\" onClick=\"edit_rule(\'"+DataArray[i].Name+"\', \'"+DataArray[i].Sub_num+"\', \'"+DataArray[i].Device+"\', \'"+DataArray[i].Access_path+"\', \'"+DataArray[i].Permission+"\')\" style=\"cursor:hand\"></td><td width=\"5%\"></td>";
					else
					{
				...
				else
				{
					if(DataArray[i].Access_path != "None")
					{
						...
						icon += "<td align=\"center\" width=\"5%\"><img src=\"modify.png\" id=\"modify\" width=\"20px\" onClick=\"edit_rule(\'"+DataArray[i].Name+"\', \'"+DataArray[i].Sub_num+"\', \'"+DataArray[i].Device+"\', \'"+DataArray[i].Access_path+"\', \'"+DataArray[i].Permission+"\')\" style=\"cursor:hand\"></td><td align=\"right\" width=\"5%\">";
				}	
