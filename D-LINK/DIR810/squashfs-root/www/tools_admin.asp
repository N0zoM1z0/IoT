...
<script language="JavaScript">
	...
	var isMyDlinkReg = (config_val("igd_Register_st_")? config_val("igd_Register_st_"):"");
		...
		if($('#hostname').val() != ori_hostname)
		{
			...
			if(submit_button_flag == 0) {
				...
				var param = {
					...
					//2014-02-11 Silvia, Support WFA 1.00 need to send ccpSubEvent3 for change samba password
					if ($('#admin_password').val() != array_password[0])
					{
						if (is_WFA100)
							param.arg = "ccp_act=set&ccpSubEvent=CCP_SUB_ADMIN&ccpSubEvent2=CCP_SUB_REMOTEMNGR&ccpSubEvent3=CCP_SUB_WEBACCESS_SMB_PASSWD&nextPage=login.asp";
						else
							param.arg = "ccp_act=set&ccpSubEvent=CCP_SUB_ADMIN&ccpSubEvent2=CCP_SUB_REMOTEMNGR&nextPage=login.asp";
					} else
						param.arg = "ccp_act=set&ccpSubEvent=CCP_SUB_REMOTEMNGR&nextPage=tools_admin.asp";
