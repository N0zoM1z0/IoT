...
<script language="JavaScript">
	...
	function paintDNSTable()
	{
		...
		for(var i = 0; i < DataArray.length; i++){
			...
			contain += "<tr><td><center><input type=checkbox value=1 id=r_enable" + i + " name=r_enable" + i + " onClick='set_reserved_enable(" + i + ")' " + is_enable + "></center></td><td><center>" + DataArray[i].Name +"</td><td><center>"+ DataArray[i].IP +"</center></td><td><center><a href=\"javascript:edit_row("+ i +")\"><img src=\"edit.jpg\" border=\"0\" title=\""+get_words('_edit')+"\"></a></center></td><td><center><a href=\"javascript:del_row(" + i +")\"><img src=\"delete.jpg\"  border=\"0\" title=\""+get_words('_delete')+"\"></a></center></td></tr>";
		}
		contain += '</table>';
...
*/			}
	...
	function send_request()
	{
		...
		if (get_by_id('d_enable').checked)
		{
			...
			// 2013-04-11 Silvia add, Check Host name entry
			if (!check_client_name(host_name) || _isNumeric(host_name))
			{
				alert(get_words('DDNS_HOST_ERROR'));
				return false;
			}
