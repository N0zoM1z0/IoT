			...
			if (wep_def_key[i-1].selectedIndex){
		    	...
		    	if (key.length != wep_key_len && key.length != hex_len){
			    		...
			    		if(def_key != i)	//add by Vic for avoid check wep key when the index is not selected
							continue;
						alert(which_lang[TEXT041_1]+" " + i + " "+which_lang[TEXT041_2]+" " + wep_key_len + " "+which_lang[TEXT041_3]+" " + hex_len + " "+which_lang[TEXT041_4]);//TEXT041			    		
			    		return false;
		    	}else if(key.length == hex_len){
			      	for (var j = 0; j < key.length; j++){
			      		if (!check_hex(key.substring(j, j+1))){
			      			//alert(TEXT042_1+" " + i + " "+TEXT042_2);//TEXT042
			      			alert(which_lang[TEXT042_1]+" " + i + " "+which_lang[TEXT042_2]);//TEXT042
			      			if(def_key != i)	//add by Vic for avoid check wep key when the index is not selected
								continue;
			      			return false;
			      		}
	...
	for(var i = 1; i < 5; i++){
			...
			if (wep_def_key[i-1].selectedIndex){
		    	...
		    	if (key.length != wep_key_len && key.length != hex_len){
			    		...
			    		if(def_key != i)	//add by Vic for avoid check wep key when the index is not selected
							continue;
						alert(which_lang[KR16] +" " + which_lang[wwl_WK] + " "+which_lang[TEXT041_2]+" " + wep_key_len + " "+which_lang[TEXT041_3]+" " + hex_len + " "+which_lang[TEXT041_4]);//TEXT041
			    		return false;
		    	}else if(key.length == hex_len){
			      	for (var j = 0; j < key.length; j++){
			      		if (!check_hex(key.substring(j, j+1))){
			      			//alert(TEXT042_1+" " + i + " "+TEXT042_2);//TEXT042
			      			alert(which_lang[KR16] +" " + which_lang[wwl_WK]+" " + which_lang[KR22] + " "+which_lang[TEXT042_2]);//TEXT042
			      			if(def_key != i)	//add by Vic for avoid check wep key when the index is not selected
								continue;
			      			return false;
			      		}
...
function check_key_1()
{
	...
	for(var i = 1; i < 2; i++){
		...
		if (wep_def_key[i-1].selectedIndex){
			...
			if (key.length != wep_key_len && key.length != hex_len){
				alert(which_lang[KR17] +" " + which_lang[wwl_WK] + " "+which_lang[TEXT041_2]+" " + wep_key_len + " "+which_lang[TEXT041_3]+" " + hex_len + " "+which_lang[TEXT041_4]);//TEXT041
				return false;
			}else if(key.length == hex_len){
				for (var j = 0; j < key.length; j++){
					if (!check_hex(key.substring(j, j+1))){
					alert(which_lang[KR17] +" " + which_lang[wwl_WK] + " "+which_lang[TEXT042_2]);//TEXT042
						return false;
					}
				}
				if(i == def_key)
	...
	for(var i = 1; i < 2; i++){
			...
			if (wep_def_key[i-1].selectedIndex){
		    	...
		    	if (key.length != wep_key_len && key.length != hex_len){
			      	...
			      	for (var j = 0; j < key.length; j++){
			      		if (!check_hex(key.substring(j, j+1))){
			      			alert(which_lang[KR16] +" " + which_lang[wwl_WK]+ " "+which_lang[TEXT042_2]);//TEXT042
							if(def_key != i)	//add by Vic for avoid check wep key when the index is not selected
								continue;
...
function check_lan_setting(ip, mask, gateway, obj_word){
	 ...
	 if (!check_mask(mask)){
		...
		alert(addstr(get_words('TEXT043'), obj_word, gateway_ipaddr_1));
		return false;
	}
	return true;
}
...
function check_address(my_obj, mask_obj, ip_obj){
	...
	if (my_obj.addr.length == 4){
		...
		if (((my_obj.addr[0] == "255") && (my_obj.addr[1] == "255") && (my_obj.addr[2] == "255") && (my_obj.addr[3] == "255") && !my_obj.is_udp) ||
			((my_obj.addr[0] >= 240) && (my_obj.addr[0] <= 255) && !my_obj.is_udp)
		)
		{
			alert(my_obj.e_msg[INVALID_IP]);
			return false;
		...
		if (!my_obj.allow_zero && count_zero == 4){	// if the ip is not allowed to be 0.0.0.0
			...
			if (check_address.arguments.length >= 2 && mask_obj != null){
				...
				if(my_obj.is_udp)
					mask = new Array(255,255,255,255);
				else
...
function copy_virtual(index){
	...
	if (get_by_id("application" + index).selectedIndex > 0){
		...
		alert(which_lang[GW_NAT_NAME_INVALID]);
	}
}
 ...
 */
 function page_cancel_1(form_name, redirect_url){
	if (!is_form_modified(form_name) && confirm (which_lang[up_fm_dc_1])) {
		setTimeout(function() {
