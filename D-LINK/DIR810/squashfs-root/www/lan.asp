...
<script language="JavaScript">
		...
		if(check_mask(temp_mask_obj) && check_address(temp_ip_obj, temp_mask_obj)){
			...
			if(submit_button_flag == 0){
				...
				else
				{
					...
					var reboot_param = {
						url: "cfg_op.ccp",
						arg: "ccp_act=reboot&fromLan=1&newIP="+$('#lan_ipaddr').val()
					};
					get_config_obj(reboot_param);
				}
	...
	function paintReservedTable()
	{
		...
		for(var i = 0; i < DataArray.length; i++){
			...
			contain += "<tr><td><center><input type=checkbox value=1 id=r_enable" + i + " name=r_enable" + i + " onClick='set_reserved_enable(" + i + ")' " + is_enable + "></center></td><td><center>" + sp_words(DataArray[i].Name) +"</td><td><center>" + DataArray[i].MAC +"</center></td><td><center>"+ DataArray[i].IP +"</center></td><td><center><a href=\"javascript:edit_row("+ i +")\"><img src=\"edit.jpg\" border=\"0\" title=\""+get_words('_edit')+"\"></a></center></td><td><center><a href=\"javascript:del_row(" + i +")\"><img src=\"delete.jpg\"  border=\"0\" title=\""+get_words('_delete')+"\"></a></center></td></tr>";
		}
		contain += '</table>';
	...
	function copyDataToDataModelFormat()
	{
		...
		if((get_by_id("old_ip").value != get_by_id("new_ip").value) ||
			...
			(get_by_id("old_dnsrelay").value != get_by_id("new_dnsrelay").value) ||
			(get_by_id("old_dhcpd").value != get_by_id("new_dhcpd").value))
		{
	...
	function ajax_submit(p,as)
	{
		var time=new Date().getTime();
		var ajax_param = {
			type: 	"POST",
			async:	(as==undefined?true:as),
			url: 	p.url,
