...
<script type="text/javascript">
	...
	**/
	...
	var lanCfg = {
		...
		'dfs':				config_str_multi("lanWlanCfg_DFSEnable_")
...
*/
	function onPageLoad()
	{
		...
		show_chan_width_0();
		$('#11n_protection').val(lanCfg.chanwidth[0]);
		...
		/* 
		 * 20130104 moa add when choose not ac mode, hide 80MHz
		 */
		var check80_func = 
		function()
		{
			...
			{
				$('#11a_protection option[value=2]').remove();
			}
			else
			{
				if($('#11a_protection option[value=2]').length==0)
				{
					$('#11a_protection').append('<option value="2">'+get_words('bwl_ht204080')+'</option>');
				}
			}
		};
   ...
   function show_chan_width_0()
   {
		...
		**/
		...
		if (mode =='11g' || mode =='11bg')
		{
			...
			$('#11n_protection').val("0");
		} else if (mode =='11n') {
			$('#show_channel_width').show();
			$('#11n_protection').val("1");
		} else
			$('#show_channel_width').show();
...
//            guest_schedule_name = "255";
	...
	function send_request()
	{
		...
		if (get_checked_value($('#w_enable')[0]) ==1)
		{
			...
			}else if(wep_type_value_0 == 1){		//WEP
	...
	//2013.06.25 Silvia modify, move channel to ncc_dev_independence
	function set_channel_1()
	{
		...
		//20130227 request to hide channel 165
		//$("#sel_wlan1_channel option[value='165']").hide();  rm it by silvia, request from Brian
	}
	...
	/**
	**    Date:		2013-07-04 / 2013-10-24
	**    Author:	Silvia Chang
	**    Reason:	DLink requirement
	**    Note:		modfiy for ch 165, channel width only can set 20. /
	**			 	Domain TW involves illegal channels, ch 56 only "20 only", ch 60 and 64 only "24/40"
	**/
	...
	function Format_basic()
	{
		...
		if (get_checked_value($('#w_enable')[0]) == 1)
		{
			...
			for (i=1;i<=2;i++)
			{
				...
				basic += "&wpsCfg_Enable_1.1."+(i+2)+".1.0="+is_wps;
				...
				basic += "&lanWlanCfg_ChannelWidth_1.1."+i+".0.0="+$('#11n_protection').val();
			}
			...
			for (i=3;i<=4;i++){
				...
				switch($('#dot11_mode_1').val())
				{
					...
					case "11na":
						...
						break;
					case "11acna":
						basic += "&lanWlanCfg_Standard5G_1.1."+i+".0.0=3";
						break;
					case "11acn":
						basic += "&lanWlanCfg_Standard5G_1.1."+i+".0.0=4";
						break;
					case "11ac":
						basic += "&lanWlanCfg_Standard5G_1.1."+i+".0.0=5";
						break;
				}
				basic += "&lanWlanCfg_ChannelWidth_1.1."+i+".0.0="+$('#11a_protection').val();
		...
		<div class="box">
			...
			<tr> 
				...
				<td width="340">&nbsp;&nbsp;
					...
					<option value="11ac"><script>show_words(bwl_Mode_ac)</script></option>
					<option value="11na"><script>show_words(bwl_Mode_5)</script></option>
					<option value="11acn"><script>show_words(bwl_Mode_acn)</script></option>
					<option value="11acna"><script>show_words(bwl_Mode_acna)</script></option>
			...
			<tr id="show_channel_width_1"> 
				...
				<td width="340">&nbsp;&nbsp;
					...
					<option value="0"><script>show_words(bwl_ht20)</script></option>
					<option value="1"><script>show_words(bwl_ht2040)</script></option>
					<option value="2"><script>show_words(bwl_ht204080)</script></option>
					</select> 
