...
<head>
	...
	var param = {
		...
		arg: 	"ccp_act=get&num_inst=10&"+
				...
				"oid_10=IGD_WANDevice_i_WANConnectionDevice_i_&inst_10=11100"
	};
	...
	var array_IsWOL			= config_str_multi("vsRule_IsWakeOnLan_");
	...
	function send_request(){
		...
		var lanip_addr_msg = replace_msg(all_ip_addr_msg,which_lang[help256]);
		var lanmask_addr_msg = replace_msg(all_ip_addr_msg,which_lang[help256]);
		...
		for (var i = 0; i < rule_max_num; i++) {
			...
			if (check_name != ""){
				...
				if(!check_LAN_ip(lan_ip.split('.'), temp_ip_obj.addr, which_lang[help256])){
					return false;
				}
    ...
    <table id="header_container" border="0" cellpadding="5" cellspacing="0" width="838" align="center">
      <tr>
        <td width="100%">&nbsp;&nbsp;<script>show_words('TA2')</script>: 
            <a href="http://www.dlink.com/us/en/support" onclick="return jump_if();"><script>document.write(model);</script></a></td>
    ...
    <table id="topnav_container" border="1" cellpadding="2" cellspacing="0" width="838" height="100%" align="center" bgcolor="#FFFFFF" bordercolordark="#FFFFFF">        <tr>
		...
		<form id="form1" name="form1" method="post" action="get_set.ccp">
				...
				for(var i=1; i<=rule_max_num; i++)
				{
					...
					document.write('<input type="hidden" id="vsRule_IsWakeOnLan_'+i+'" name="vsRule_IsWakeOnLan_1.1.'+i+'.0.0" value="">');
				}
				</script>
