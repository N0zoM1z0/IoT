...
<script language="JavaScript">
	...
	function send_request(){
		...
		submitData += "&igdStorage_Http_Remote_Access_Enable_1.1.1.0.0=0";
		...
		if(user_inst == null)
		{
			...
			submitData += '&igdStorageUserRule_AccessPath_1.1.1.1.0='+ encodeURIComponent($('#folder_path').val());
		...
		{			
			...
			else
			{
					...
					submitData += '&igdStorageUserRule_AccessPath_1.1.'+(null_count)+'.1.0=' + encodeURIComponent($('#folder_path').val());
...
//					' clr="req_ctx(\''+rPath+folders[i]+'\', \''+transUid(ulId, folders[i])+'\')">'+
	...
	function chk_ddnslink()
	{
		...
		var ret = config_val("DdnsCheckResult");
