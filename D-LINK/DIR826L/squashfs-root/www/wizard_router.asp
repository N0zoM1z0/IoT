...
<script language="JavaScript">
	...
	var pageNameArray = new Array('p0', 'p1', 'p2', 'p3', 'p4', 'p5', 'p6', 'p6a', 'p6b', 'p7a', 
								  'p7b', 'p7c', 'p7d', 'p7e', 'p7f', 'p7g', 'p8', 'p9', 'p10', 'p11',
								  'p12', 'p13','p13', 'p13a','p13b','p14', 'p15', 'Unknown');
	var nextPageArray = new Array('p3', 'p2', 'p9', 'p4', 'p5', 'p6', 'p7a', 'p8', 'p8', 'p8', 
								  'p8', 'p8', 'p8', 'p8', 'p8', 'p8', 'p1', 'p10', 'p12', 'p12',
								  'p13','p13a','p13b', 'p14','p14','p14', 'p15','Unknown');
	...
	var	param = {
		...
		arg: "oid_1=IGD_&inst_1=10000"+
			...
			"&oid_9=IGD_WANDevice_i_WANConnectionDevice_i_PPTP_i_&inst_9=11110"+
			"&oid_10=IGD_WANDevice_i_WANConnectionDevice_i_PPTP_i_ConnectionCfg_i_&inst_10=11111"+
			"&oid_11=IGD_WANDevice_i_WANConnectionDevice_i_L2TP_i_&inst_11=11110"+
			"&oid_12=IGD_WANDevice_i_WANConnectionDevice_i_L2TP_i_ConnectionCfg_i_&inst_12=11111"+
			"&oid_13=IGD_AdministratorSettings_&inst_13=10000"
	};

	var j = 13
	for (i=1;i<=4;i++){
		param.arg +="&oid_"+(j+1)+"=IGD_LANDevice_i_WLANConfiguration_i_&inst_"+(j+1)+"=11"+i+"00";
		param.arg +="&oid_"+(j+2)+"=IGD_LANDevice_i_WLANConfiguration_i_WPA_PSK_&inst_"+(j+2)+"=11"+i+"11";
		j+=2;
	}

	param.arg += "&ccp_act=get&es_step=0&num_inst="+j;
	get_config_obj(param);

	var wlanPASS = config_str_multi('wpaPSK_KeyPassphrase_');
	var wlanSSID = config_str_multi('lanWlanCfg_SSID_');
	...
	var wanCfg = {
		...
		'wanStaticDns':		config_val('staticIPCfg_DNSServers_').split(','),
		...
		'wanPppoeDNS':		config_val('pppoeCfg_DNSServers_').split(','),
		...
		'wanDhcpPass':		config_val('dhcpplusCfg_Password_')? config_val('dhcpplusCfg_Password_'):"",
		...
		'wanPPTPDNS':		config_val('pptpCfg_DNSServers_').split(','),
		...
		'wanL2TPDNS':		config_val('l2tpCfg_DNSServers_').split(',')
	};
	...
	{
		var paramStr = "";
		paramStr += "&ccpSubEvent=CCP_SUB_SETUP_WIZARD"; //CCP_SUB_WIZARD change to CCP_SUB_SETUP_WIZARD
		paramStr += "&nextPage=sel_wan.asp";
		paramStr += "&loginInfo_Username_1.1.1.0.0=admin&loginInfo_Password_1.1.1.0.0=" + urlencode($('#pwd1').val());
		paramStr += "&adminCfg_GraphAuthenticationEnable_1.1.0.0.0=" + get_checked_value(get_by_id("graphical_enable"));
		paramStr += "&sysTime_LocalTimeZone_1.1.0.0.0=" + $('#tzone').val();
		paramStr += "&sysTime_LocalTimeZoneName_1.1.0.0.0=" + $('#tzone')[0].options[$('#tzone')[0].selectedIndex].text;
		paramStr += "&sysTime_LocalTimeZoneIndex_1.1.0.0.0=" + $('#tzone')[0].selectedIndex;

		paramStr += "&sysTime_NTPEnable_1.1.0.0.0=1";
		paramStr += "&sysTime_NTPServer1_1.1.0.0.0=ntp1.dlink.com";
	...
	{
		var paramStr = "";
		paramStr += "&ccpSubEvent=CCP_SUB_WIRELESS"; 
		paramStr += constructParamWifi(); 
	...
	function submit_regist(val)
	{
		var paramStr={
			'url': 	'easy_setup.ccp',
			'arg': 	'ccp_act=set&ccpSubEvent=CCP_SUB_MYDLINK'
		};
		//20120203 silvia add if user do the registration then change defcfg
		paramStr.arg += '&adminCfg_HttpsServerEnable_1.1.0.0.0=1';
				...
				switch(action)
				{
					...
					case 2:	//adddev
						...
						{							
							...
							{
								//next_page(findIndexOfArrayByValue(pageNameArray, 'p15'));
								submit_wifi(1);
							}else{
								get_conn_st_mydlink();
								//setTimeout('OpenWindow()', 1000 * 2);
								//setTimeout('location.replace(\'login.asp\')', 1000 * 12);
							}
							//OpenWindow();
							//submit_wifi();
	...
	function wizard_cancel(wz){
		...
		//for submit wifi
		if(wz==3 && ( wlanSSID[0] != $('#p8_ssid').val() || wlanSSID[2] != $('#p8_ssid_5').val() || 
						wlanPASS[0] != $('#p8_key').val() || wlanPASS[2] != $('#p8_key_5').val()))
			submit_wifi(0);			
				...
				if (pageNameArray[i] == 'p12')
				{
					var param = {
						...
						arg: "ccp_act=get&num_inst=1"+
							'&oid_1=IGD_WANDevice_i_WANConnectionDevice_i_&inst_1=11100'
					};
					get_config_obj(param);
					conn_type = config_val("wanConnDev_CurrentConnObjType_");
	...
	function preload_wz_page_p3()
	{
		var paramHost={
			'url': 	'easy_setup.ccp',
			'arg': 	'ccp_act=get&es_step=0&num_inst=1&oid_1=IGD_Status_WANStatus_&inst_1=11000'
		};
		get_config_obj(paramHost);

		var cableSt = config_val('igdWanStatus_CableStatus_');
	...
	function verify_wz_page_p7g()
	{
		var c_host = $('#dhcpplus_host').val();
		/*
		 * Validate MAC and activate cloning if necessary
		 */

		var mac = $('#dhcpplus_wan_mac').val();
		if (!check_mac(mac)){
			alert (which_lang[KR3]+":" + mac + ".");
			return false;
		}

		var mac = trim_string($('#dhcpplus_wan_mac').val());
		if(!is_mac_valid(mac, true)) {
			alert (get_words('KR3')+":" + mac + ".");
			return false;
		}
		...
		if($('#dhcpplus_userid').val() == ""){
    		alert(get_words('GW_WAN_DHCPPLUS_USERNAME_INVALID'));
    		return false;
	    }

	    if ($('#dhcpplus_pwd').val() == "" || $('#dhcpplus_pwd2').val() == ""){
...
*/
	...
	function constructParamWifi()
	{
		var paramStr = "";

		paramStr += "&lanWlanCfg_Enable_1.1.1.0.0=1&lanWlanCfg_SSID_1.1.1.0.0="+urlencode($('#p8_ssid').val());
		paramStr += "&lanWlanCfg_Enable_1.1.3.0.0=1&lanWlanCfg_SSID_1.1.3.0.0="+urlencode($('#p8_ssid_5').val());

		for (var i =1;i<=3;)
		{
			paramStr += "&lanWlanCfg_SecurityMode_1.1."+i+".0.0=2";
			paramStr += "&wpaInfo_WPAMode_1.1."+i+".1.0=0";
			paramStr += "&wpaInfo_AuthenticationMode_1.1."+i+".1.0=0";
			paramStr += "&wpaInfo_EncryptionMode_1.1."+i+".1.0=2";

			// since security will be modified, WPS should always be set configured.
			paramStr += "&wpsCfg_Status_1.1."+i+".1.0=1";
			i+=2;
		}

		paramStr += "&wpaPSK_KeyPassphrase_1.1.1.1.1="+urlencode($('#p8_key').val());
		paramStr += "&wpaPSK_KeyPassphrase_1.1.3.1.1="+urlencode($('#p8_key_5').val());
		paramStr += "&wpsCfg_Status_1.1.1.1.0=1";
		return paramStr;
	}
	...
	function constructParamDHCPC()
	{
		...
		paramStr += "&wanConnDev_MACAddress_1.1.1.0.0=" + $('#wan_mac').val();
	...
	function constructParamDHCPPlus()
	{
		...
		paramStr += "&wanConnDev_MACAddress_1.1.1.0.0=" + $('#dhcpplus_wan_mac').val();
		paramStr += "&wanConnDev_MACAddressOverride_1.1.1.0.0=" + already_clone;
		paramStr += "&dhcpplusCfg_Name_1.1.1.1.0=" + $('#dhcpplus_host').val();
		paramStr += "&dhcpplusCfg_Username_1.1.1.1.0=" + urlencode($('#dhcpplus_userid').val());
		paramStr += "&dhcpplusCfg_Password_1.1.1.1.0=" + urlencode($('#dhcpplus_pwd').val());
	...
	function constructParamPPPoE()
	{
		...
		paramStr += "&pppoeCfg_Username_1.1.1.1.0=" + urlencode($('#pppoe_user_name').val());
		paramStr += "&pppoeCfg_Password_1.1.1.1.0=" + urlencode($('#pppoe_pwd1').val());
		paramStr += "&pppoeCfg_NetSniperSupport_1.1.1.1.0=0";
		paramStr += "&pppoeCfg_SpecialDialMode_1.1.1.1.0=0";
		paramStr += "&pppoeCfg_PPPoEPlusDialEnable_1.1.1.1.0=" + get_checked_value($('#pppoe_plus_support')[0]);
		return paramStr;
	}
	...
	function constructParamStaticIP()
	{
		...
		paramStr += "&staticIPCfg_ExternalIPAddress_1.1.1.1.0=" + $('#ip').val();
		paramStr += "&staticIPCfg_SubnetMask_1.1.1.1.0=" + $('#mask').val();
		paramStr += "&staticIPCfg_DefaultGateway_1.1.1.1.0=" + $('#gateway').val();	
		paramStr += "&staticIPCfg_DNSServers_1.1.1.1.0=" + $('#static_dns1').val() +","+ $('#static_dns2').val();
	...
	function constructParamPPTP()
	{
		...
		paramStr += "&pptpCfg_ExternalIPAddress_1.1.1.1.0=" + $('#pptp_ip').val();
		paramStr += "&pptpCfg_SubnetMask_1.1.1.1.0=" + $('#pptp_mask').val();
		paramStr += "&pptpCfg_DefaultGateway_1.1.1.1.0=" + $('#pptp_gw').val();
		paramStr += "&pptpCfg_DNSServers_1.1.1.1.0=" + $('#pptp_dns1').val() + ',' + $('#pptp_dns2').val();
		paramStr += "&pptpConn_ServerIP_1.1.1.1.1=" + $('#pptp_server_ip').val();
		paramStr += "&pptpConn_Username_1.1.1.1.1=" + urlencode($('#pptpuserid').val());
		paramStr += "&pptpConn_Password_1.1.1.1.1=" + urlencode($('#pptppwd').val());
		return paramStr;
	}
	...
	function constructParamL2TP()
	{
		...
		paramStr += "&l2tpCfg_ExternalIPAddress_1.1.1.1.0=" + $('#l2tp_ip').val();
		paramStr += "&l2tpCfg_SubnetMask_1.1.1.1.0=" + $('#l2tp_mas').val();
		paramStr += "&l2tpCfg_DefaultGateway_1.1.1.1.0=" + $('#l2tp_gw').val();
		paramStr += "&l2tpCfg_DNSServers_1.1.1.1.0=" + $('#l2tp_dns1').val() + ',' + $('#l2tp_dns2').val();
		paramStr += "&l2tpConn_ServerIP_1.1.1.1.1=" + $('#l2tp_server_ip').val();
		paramStr += "&l2tpConn_Username_1.1.1.1.1=" + urlencode($('#l2tpuserid').val());
		paramStr += "&l2tpConn_Password_1.1.1.1.1=" + urlencode($('#l2tppwd').val());
		return paramStr;
	}
	...
	$(document).ready( function () {
		...
		$('#wan_mac').val(wanCfg.wanMac);
		$('#dhcpplus_host').val(wanCfg.wanDhcpplusHost);
		$('#dhcpplus_userid').val(wanCfg.wanDhcpplusUser);
...
*/
	function do_sp_ping()
	{
		var paramPing = {
			url: "ping.ccp",
			arg: 'ccp_act=ping_v4&iface=lan&ping_addr=http://www.dlink.com'
		};
		ping_wan(paramPing);
		setTimeout('do_sp_ping()', pingIntv);
	}
...
<!-------------------------------->
		...
		<tr>
			...
			<td>&nbsp;
				<input type="password" id=pwd1 name=pwd1 size=20 maxlength=15 value=''>
			</td>
		</tr>
		<tr>
			<td align=right class="duple"><script>show_words(_verifypw)</script> :</td>
			<td>&nbsp;
				<input type="password" id=pwd2 name=pwd2 size=20 maxlength=15 value=''>
			</td>
			</tr>
			<tr>
...
<!-------------------------------->
		<div class=box id="p3" style="display:none"> 
			...
			<div align="center">
				<img src="netowrk-topology.png" width="603" height="246">
			</div>
			<div> 
				<table align="center" class="formarea">
				<tr>
				...
				<div>
					...
					<tr>
						<td align=right class="duple">
							...
							&nbsp;:</td>
						<td>
							<input type=text id=pptp_server_ip name=pptp_server_ip size="20" maxlength="15" value=''>
						</td>
					</tr>
					<tr>
						<td align=right class="duple">
		...
		<div class=box id="p7e" style="display:none">
			...
			<div align="left"> 
				...
				<div>
					<table width="536" align="center" class=formarea>
					<tr>
						<td width="240" align=right class="duple3">
...
<!-------------------------------->
<!--      Start of page 7f (bigpond)     -->
<!-------------------------------->
		<div class=box ID="p7f" style="display:none"> 
			<h2 align="left"><script>show_words(wwa_title_set_bigpond)</script></h2>
			<p class="box_msg"><script>show_words(wwa_msg_set_bigpond)</script></p>
				...
				<tr>
					<td></td>
					<td>
						<input type="button" class="button_submit" id="cancel_b_p7f" name="cancel_b_p7f" value="" onclick="wizard_cancel();">
						<input type="button" class="button_submit" id="wz_prev_b_p7f" name="wz_prev_b_p7f" value="" onclick="prev_page();"> 
						<input type="button" class="button_submit" id="next_b_p7f" name="next_b_p7f" value="" onClick="next_page();"> 						
						<script>$('#wz_prev_b_p7f').val(get_words('_prev'));</script>
						<script>$('#next_b_p7f').val(get_words('_next'));</script>
						<script>$('#cancel_b_p7f').val(get_words('_cancel'));</script>
					</td>
				</tr>
				</table>
		</div>
...
<!-------------------------------->
<!--      Start of page 7g (DHCP-plus)     -->
<!-------------------------------->
		<div class=box id="p7g" style="display:none"> 
			<h2 align="left"><script>show_words(_dhcpconn)</script></h2>
			<div align="left"> 
				<p class="box_msg"><script>show_words(wwa_msg_set_dhcp)</script></p>
				<div>
					...
					<tr>
						...
						<td>
							<input type="button" class="button_submit" id="cancel_b_p7g" name="cancel_b_p7g" value="" onclick="wizard_cancel();"> 
							<input type="button" class="button_submit" id="wz_prev_b_p7g" name="wz_prev_b_p7g" value="" onclick="prev_page();"> 
							<input type="button" class="button_submit" id="next_b_p7g" name="next_b_p7g" value="" onClick="next_page();"> 							
							<script>$('#wz_prev_b_p7g').val(get_words('_prev'));</script>
							<script>$('#next_b_p7g').val(get_words('_next'));</script>
							<script>$('#cancel_b_p7g').val(get_words('_cancel'));</script>
						</td>
					</tr>
					</table>
				</div>
...
<!-------------------------------->
		<div class="box" id="p8" style="display:none"> 
			...
			<table align="center" cellpadding="5" class=formarea style="display:"> 
			<div align="left" width="300" height="240">
			<tr>
				<td width="300" height="30">
					<p class="box_msg" align="left"> 
						<script>show_words(wwz_wwl_intro_s0)</script>&nbsp
			...
			<tr align="left"> 
				<td colspan="2" class=form_label> 
					<b><script>show_words(wwz_wwl_intro_s2_1_1)</script> : </b><br>
					<input type="text" id="p8_ssid" name="p8_ssid" size="40" maxlength="32" value="">
					<script>show_words(wwz_wwl_intro_s2_1_2)</script>
				</td> 
			</tr><br>

			<tr align="left"> 
				<td colspan="2" class=form_label> 
					<b><script>show_words(wwz_wwl_intro_s2_2_1)</script> : </b><br>
					<input type="text" id="p8_key" name="p8_key" size="40" maxlength="63" value="">
					<input type="hidden" id="p8_autokey" name="p8_key" size="40" maxlength="63" value=""> 
					<script>show_words(wwz_wwl_intro_s2_2_2)</script>
				</td>
			</tr>
		...
		<!-- 20120112 silvia add 5g -->
			...
			<tr align="left"> 
				<td colspan="2" class=form_label> 
					<b><script>show_words(wwz_wwl_intro_s2_1_1)</script> : </b><br>
					<input type="text" id="p8_ssid_5" name="p8_ssid_5" size="40" maxlength="32" value="">
					<script>show_words(wwz_wwl_intro_s2_1_2)</script>
				</td> 
			</tr><br>
			...
			<tr align="left"> 
				<td colspan="2" class=form_label> 
					<b><script>show_words(wwz_wwl_intro_s2_2_1)</script> : </b><br>
					<input type="text" id="p8_key_5" name="p8_key_5" size="40" maxlength="63" value="">
		...
		<div class=box id="p9" style="display:none">
			...
			<table align="center" class=formarea style="display:"width="94%"> 
				...
				<tr><td>
					...
					<br>
					<table align="center" width="70%" height="80" cellspacing="14">
					<div align="left">
					<tr> 
						<td class="duple3">
		...
		<table width="98%" height="70%" border="0" align="center" bgcolor="#FFFFFF" cellspacing="5" >
			...
			<tr>
				...
				&nbsp;&nbsp;<input type="button" id="append_cancel1" name="append_cancel1" onClick="wz_back(0)" value="">
					<script>$('#append_cancel1').val(get_words('_cancel'));</script>
				</td>
			</tr>
		...
		<h2 align="left"><script>show_words(ES_title_s6)</script></h2>
			<table class=formarea>
				<tr height="24"><br>
					...
					<td width="45%">
						<input type="text" id="email_addra" name="email_addra" value="" tabindex="120">
					</td>
				</tr>
				<tr height="24"><br>
					<td></td>
					...
					<td>
						<input type="password" id="pass" name="pass" value="" tabindex="120">
					</td>
				</tr>
			</table><br>
		...
		<center><p class="box_msg"><script>show_words(mydlink_tx10)</script></p></center>
			<table class=formarea>
				<tr height="24"><br>
					...
					<td width="52%">
						<input type="text" id="email_addr" name="email_addr" value="" tabindex="120">
					</td>
				</tr>
				<tr height="24">
					<td></td>
					<td><div align="right"><script>show_words(_password)</script>&nbsp;:&nbsp;</div></td>
					<td>
						<input type="password" id="passwd" name="passwd" value="" tabindex="120">
					</td>
				</tr>
				<tr height="24">
					<td></td>
...
<!-------------------------------->
		...
		<h2 align="left"><script>show_words(mydlink_WiFi_save)</script></h2>
		<table border=0 align="center" cellPadding=0 cellSpacing=0 >
		<tr>
			<td ><p>&nbsp;</p>
			<table width="650" border="0" align="center">
			<tr>
				<td height="15">
				<div>
					<p class="centered"><script>show_words(mydlink_WiFi_save)</script></p>
					<p></p>
					<p class="centered"><script>show_words(_please_wait)</script></p>
					<p></p>
				</div>
