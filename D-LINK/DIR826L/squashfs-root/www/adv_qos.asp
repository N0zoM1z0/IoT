...
<script language="JavaScript">
	...
	function send_request()
	{
		...
		if ($("#qos_traffic_shaping")[0].checked) {
			...
			//20120509 silvia modify max val and is not a int
			if (!(check_integer($("#qos_uplink").val(), 8, 10240000))) {
				alert(get_words('at_alert_1_1'));
				return false;
			}
		}
            ...
            for (var j = 0; j < rule_max_num; j++) {
                if (j != i) {
					...
					if ((check_name == check_name_chk) && (check_name) && (check_name_chk)){
						alert(get_words('ag_conflict6') +' (' + $('#name' + j).val() +').');
						return false;
					}
	...
	function sendAjax(){
		if((get_checked_value($("#qos_traffic_shaping")[0]) == "1"))
			hw_nat_enable = "0";
//		else if(spi_enable == "0")
//			hw_nat_enable = "1";

		var qos_enabled = get_checked_value($("#qos_engine_enabled")[0]);

		var str="ccp_act=set&ccpSubEvent=CCP_SUB_TRAFFICTL&nextPage=adv_qos.asp" +
...
<body onload="onPageLoad();" topmargin="1" leftmargin="0" rightmargin="0" bgcolor="#757575">
    ...
    <table id="topnav_container" border="1" cellpadding="2" cellspacing="0" width="838" height="100%" align="center" bgcolor="#FFFFFF" bordercolordark="#FFFFFF">        <tr>
            ...
            <form id="form1" name="form1" method="post" action="apply_qos.cgi">
                ...
                <div id="maincontent">
                  ...
                  <div class=box>
                    ...
                    <tr>
                        ...
                        <td>&nbsp;
							<input type="text" id="qos_uplink" name="qos_uplink" maxlength="8" size="8"  value=''>&nbsp;
