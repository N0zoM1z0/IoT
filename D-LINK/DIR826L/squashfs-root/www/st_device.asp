...
/*
	...
	*/
	...
	function getDeviceStatus() {
		...
		get_wan_time(config_val('igdWanStatus_WanUpTime_'));
		$('#show_uptime').html();
...
*/						$('#wlan'+j+'_security_gz').html(get_words('_WEP'));//+ " " +authTypeStr+" "+keyLenStr);
			...
			{
				i+=1;
				$('#show_wlan'+j).html(get_words('_disabled'));
				// end of silvia add
			}
		}
		set_control_button();
			...
			{
				{
					...
					else if($("#network_status").html() == get_words('DISCONNECTING'))
						$("#disconnect").attr("disabled",true);
					else
					{
	...
	function do_connect()
	{
		$("#network_status").html("Connecting");
		$("#connect").attr("disabled",true);
		var paramConnect = {
			url: "get_set.ccp",
	...
	{
		$("#network_status").html("Disconnecting");
		$("#disconnect").attr("disabled",true);
		var paramDisconnect = {
			url: "get_set.ccp",
			...
			<td valign="top" id="maincontent_container">
				<div id="maincontent">
					...
					<div class="box">
						...
						<tr> 
							<td class="duple"><script>show_words(sd_WRadio)</script>:</td>
							<td>&nbsp; <span id="show_wlan1"></span></td>
						</tr>
						<tr id="show_wlan1_dot11_mode">
							<td class="duple"><script>show_words(bwl_Mode)</script> :</td>
							<td>&nbsp; <span id="wlan1_mode"></span></td>
						</tr>
						<tr id="show_wlan1_11n_protection">
							<td class="duple"><script>show_words(bwl_CWM)</script>:</td>
							<td>&nbsp; <span id="wlan1_11n_prot"></span> MHz</td>
						</tr>
