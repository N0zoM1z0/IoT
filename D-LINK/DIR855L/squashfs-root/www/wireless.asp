...
var radius_button_flag_1 = 0;
	...
	function onPageLoad(){
		...
		set_selectIndex("<!--# echo wlan1_11ac_chwidth -->", get_by_id("11ac_protection"));
    ...
    function show_wpa_wep_1(){			
       ...
       function show_chan_width_1(){
		var mode = get_by_id("dot11_mode_1");
 		var mode_value = mode.options[mode.selectedIndex].value;

		if (mode_value == "11a") {
			get_by_id("show_channel_width_1").style.display = "none";
			get_by_id("11a_protection").value ="20";
			get_by_id("show_channel_width_2").style.display = "none";
			add_channel();
		} else if (mode_value.indexOf("11ac") >= 0) {
			get_by_id("show_channel_width_1").style.display = "none";
			get_by_id("show_channel_width_2").style.display = "";
			add_channel_ac();
		} else {
	...
	function disable_wireless_1(){
		...
		get_by_id("11ac_protection").disabled = !get_by_id("w_enable_1").checked;
		...
		if ("<!--# echo feature_11ac -->" != "1") { //no support for 802.11ac
			var obj = get_by_id("dot11_mode_1");
			for (var i = obj.length-1; i >= 0; i--) {
				if (obj[i].value == "11ac_only" || obj[i].value == "11ac_11n" || obj[i].value == "11ac")
					obj.remove(i); //remove 802.11ac mode
			}
		}
	}
	...
	function send_request(){	
		...
		if(get_by_id("w_enable_1").checked)
		{
			if(wep_type_value_1 == 1){		//WEP
				...
				if (wlan1_dot11_mode == "11ac" || wlan1_dot11_mode == "11ac_only" || wlan1_dot11_mode == "11ac_11n"){
					alert(MSG049);
					return false;
				}else{
					if(!check_key_1()){
				...
				if ((wlan1_dot11_mode == "11ac" || wlan1_dot11_mode == "11ac_only" || wlan1_dot11_mode == "11ac_11n") && (c_type_1_value == "tkip")){
					alert(MSG050);
					return false;
				}else{
					if (!check_varible(temp_rekey_1)){
		...
		get_by_id("wlan1_11ac_chwidth").value = get_by_id("11ac_protection").value;
...
*/
    ...
    function add_channel_ac(){
        var obj = get_by_id("sel_wlan1_channel");
	var len = obj.length - 1;
	var region = "<!--# exec cgi /bin/wlan domain number -->";
        /* The list domain channels have 140 and 165 channel */
        var domain = new Array("0x14", "0x23", "0x3C", "0x3A", "0x3B", "0x50", "0x52", "0x5B", "0x37", "0x10", "0x51");
        var protection = get_by_id("11ac_protection").value;
        for (var i = 0; i < domain.length; i++) {
                if (region.toLowerCase() == domain[i].toLowerCase()) {
                        var t_channel = obj[len].value;
            ...
            <table cellpadding="1" cellspacing="1" border="0" width="525">
              ...
              <tr> 
                ...
                <td width="340">&nbsp;&nbsp; <select id="dot11_mode_1" name="dot11_mode_1" onClick="show_chan_width_1();">
                    <option value="11a"><script type="text/javascript">show_words(bwl_Mode_a)</script></option>
                    <option value="11n"><script type="text/javascript">show_words(bwl_Mode_n)</script></option>
                    <option value="11ac_only"><script type="text/javascript">show_words(bwl_Mode_15)</script></option>
                    <option value="11na"><script type="text/javascript">show_words(bwl_Mode_5)</script></option>
                    <option value="11ac_11n"><script type="text/javascript">show_words(bwl_Mode_14)</script></option>
                    <option value="11ac"><script type="text/javascript">show_words(bwl_Mode_13)</script></option>
              ...
              <tr id="show_channel_width_2">
                <td class="duple"><script type="text/javascript">show_words(bwl_CWM)</script> :</td>
                <td width="70%" align="left">&nbsp;&nbsp;
                	<select id="11ac_protection" name="11ac_protection" onChange="add_channel_ac()">
                    <option value="0"><script type="text/javascript">show_words(bwl_ht20)</script></option>
                    <option value="1"><script type="text/javascript">show_words(bwl_ht2040_ac)</script></option>
                    <option value="2"><script type="text/javascript">show_words(bwl_ht204080_ac)</script></option>
                  </select> <input type="hidden" id="wlan1_11ac_chwidth" name="wlan1_11ac_chwidth" value="<!--# echo wlan1_11ac_chwidth -->">
                </td>
              </tr>
              <tr> 
                <td class="duple"><script>show_words(bwl_VS)</script> :</td>
