...
function mt7628_up(devname)
	...
	do
		...
		local operationmode = mtkwifi.__trim( mtkwifi.read_pipe("uci get system.route.operationmode"))
	    
		if vif == "apcli0" then
			if operationmode == "repeater" then
				os.execute("ifconfig "..vif.." up")
				os.execute("brctl addif br-lan "..vif)
			end
		end
	...
	for _,vif in mtkwifi.__spairs(string.split(mtkwifi.read_pipe("ls /sys/class/net"), "\n"))
	do
		--if string.match(vif, "apcli%d+") and cfgs.ApCliEnable ~= "0" and cfgs.ApCliEnable ~= "" then
			--os.execute("ifconfig "..vif.." up")
		--end
		local enable_apcli = mtkwifi.__trim(mtkwifi.read_pipe("wificonf -f /etc/wireless/mt7628/mt7628.dat get ApCliEnable"))
		if string.match(vif, "apcli%d+") and enable_apcli ~= "0" and enable_apcli ~= "" then
			os.execute("ifconfig "..vif.." up")
		end
	end
