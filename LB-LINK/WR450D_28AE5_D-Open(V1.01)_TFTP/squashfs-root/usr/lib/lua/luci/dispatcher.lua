...
function get_login_user()
    local opsw = require "luci.controller.admin.opsw"
    return opsw.get_login_user()
end
...
function is_first_login()
    -- old method.
    -- if file_exist("/etc/wzd_finish.flg") == 1 then return 0 else return 1 end    
    
    -- change to check nvram item of firstLogin: firstLogin == 0, need wizard; firstLogin == 1, not need wizard.
    local firstLogin_v = ""
    -- cwx: mt7628
    -- firstLogin_v = string.trim(luci.sys.exec("nvram get blink firstLogin"))
    if firstLogin_v and firstLogin_v ~= "" then
        if firstLogin_v == '0' then return 1 else return 0 end
    else
        -- there is no the nvram item, not need wizard.
        return 0
    end
end

function is_wzd_running()
    return file_exist("/tmp/wzd_running.flg")
end

function is_login_succ()
    return file_exist("/tmp/login_succ.flg")
end

function set_wzd_running_flg()
    if is_first_login()==1 then
        luci.sys.exec("touch /tmp/wzd_running.flg")
        luci.sys.exec("echo 'user' > /tmp/login_user")
        return 1
    else
        return 0
    end        
...
function get_host_domain()
    ...
    host_domain = string.trim(luci.sys.exec("nvram get blink domain"))
    if host_domain == nil then
        host_domain = ""
    end
...
function get_model_name()
    local wif_5g_exist = string.trim(luci.sys.exec("ifconfig rai0 | grep 'Link encap:Ethernet'"))
...
function authenticator.htmlauth(validator, accs, default)
	...
	local user = ""
	local pass = ""
	if is_login_succ() == 1 then
		  luci.sys.exec("rm /tmp/login_succ.flg -rf")
		  user = string.trim(luci.sys.exec("cat /tmp/login_user"))
