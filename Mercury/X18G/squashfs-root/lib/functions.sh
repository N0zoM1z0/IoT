...
load_modules() {
	[ -d /etc/modules.d ] && {
		...
		# TODO: need move this part to wifi module to support both mtk/qca product
		# add by zl
		# if DUT is caled, load modules as normal.
		# if DUT is not caled and DUT is 2g only, skip umac as ART will load it's own driver.(may cause some module load fail, but nothing serious)
		# if DUT is not caled and DUT is dualband, insmod umac with parameter.
#		if [ "$SLP_DUT_CALED" -eq "1" ]; then
#			sed 's/^[^#]/insmod &/' $* | ash 2>&- || :
#		elif [ "$ART_IS_2G_ONLY" -eq "1" ]; then
#			sed 's/^[^#]/insmod &/' $* | awk '$1 == "insmod" && $2 == "umac" {print "#" $0;} $2 != "umac" {print $0}' | ash 2>&- || :
#		else
#			sed 's/^[^#]/insmod &/' $* | awk '$1 == "insmod" && $2 == "umac" {print $0 " testmode=1 ahbskip=1";} $2 != "umac" {print $0}' | ash 2>&- || :
#		fi
		local caltag=$(/sbin/wifi get_cal_flag)
		if [ "$caltag" == "true" ]; then
			sed 's/^[^#]/insmod &/' $* | ash 2>&- || :
		else
			sed 's/^[^#]/insmod &/' $* | awk '$1 == "insmod" && $2 == "umac" {print $0 " testmode=1 ahbskip=1";} $2 != "umac" {print $0}' | ash 2>&- || :
