var component = (function($, global){
	...
	CheckBox.prototype._render = function(){
		...
		label.onclick = function(event){
			if (self.disabled || $(this).hasClass("disabled"))
			{
				return;
			}
	...
	Dialog.prototype._render = function(){
		...
		if (self._ops['size'] != Dialog.SIZE.PHONE &&
	...
	function Table(options){
		...
		this._ops = {
			...
			max: 32,
			hasFixedHeight: null,
			isPhoneScrollFixedHeight: false,	//手机端显示可滚动的表格时，需指定此值为true用来取消错误的边框及fixedHeightObj的设置
			clearAllTips: "",
			fillBlankRow: false,
			blankOptions:{
	...
	Table.prototype._render = function(){
			...
			if (self._ops.tableOptionShow)
			{
				...
				if (self._ops.tableOption != null && $.isArray(self._ops.tableOption) && self._ops.tableOption.length != 0)
				{
					for (var i = 0; i < self._ops.tableOption.length; i++)
					{
						...
						if(self._ops.disableState){
							div.className = "tableOption disabled " + tableOptionItem.type;
						}else{
				...
				if (self._ops.addable)
				{
					...
					if(!self._ops.disableState){
						...
						div.className = "tableOption add disabled";
					}
		...
		if (self._ops.hasFixedHeight != null)
		{
			...
			if (self._ops.isPhoneScrollFixedHeight) {
				$("div." + self._ops.targetId + "TableContainer").css("border-bottom", "none");
			}
		}
		...
		if (hasHead && head != null && $.isArray(head))
		{
			...
			if (self._ops.hasCheckBox)
			{
				...
				if(self._ops.hasAllCheckBox){
					...
					if(self._ops.disableState){
						self.selAllCheckBoxObj.disable(false);
					}else{
						self.selAllCheckBoxObj.enable(false);
					}
				}
			}
	...
	Table.prototype._insertRowData = function(tr, i, dataObj){
			...
			*/
			checkBox = new CheckBox({
				...
				onClick:function(){
					...
					childNum -= isAddData;
					if (self._ops.hasAllCheckBox && selAll == null)
					{
						return;
					}
					...
					self._ops.hasAllCheckBox && selAll.setValue(isSelAll);

					if (self._ops.hasAllCheckBox && self._ops.clickList.click_check &&
						typeof self._ops.clickList.click_check.func == "function")
					{
						self._ops.clickList.click_check.func(isSel, isSelAll);
					} else if (!self._ops.hasAllCheckBox) {
						self._ops.clickList.click_check.func(tr, i, dataObj);
					}
				}
			});
			if(self._ops.disableState){
		...
		for (var item = 0; item < list.length; item++)
		{
			...
			case "password":
				var passwordCon = $("<i></i>");
				if(dataObj.encryption == "1"){
					passwordCon.addClass("password iconfont icon-password");
				}
				td.appendChild(passwordCon[0]);
				break;
			case "ssidAndFreq":
				var str1,str2;
				str1 = dataObj["ssid"];
				str2 = dataObj["freqInfo"];
				td.title = str1;
				var ssidCon = $("<div></div>").addClass("ssidCon")[0];
				ssidCon.innerHTML = str1;
				td.appendChild(ssidCon);
				var freqCon = $("<div></div").addClass("freqCon")[0];
				freqCon.innerHTML = str2;
				td.appendChild(freqCon);
				break;
			case "auth":
		...
		if (this._ops.editable || this._ops.deletable || this._ops.itemOption != null)
		{
			...
			if (this._ops.editable && this._ops.disableState)
			{
				str +=  '<div class="itemOption edit disabled">' 				+
			...
			if (this._ops.deletable)
			{
				...
				if (onRender(dataObj))
				{
					if (this._ops.disableState) {
						str +=  '<div class="itemOption delete disabled">' 				+
	...
	Table.prototype._pagingListInit = function(len, startPage){
		...
		*/
		...
		if (totalPage <= 1)
		{
			...
			if (self._ops.hasFixedHeight != null && !self._ops.isPhoneScrollFixedHeight)
			{
	...
	Table.prototype._gotoPage = function(targetPage, totalPage){
		...
		if (this._ops.hasCheckBox == true)
		{
			...
			{
				self._ops.hasAllCheckBox && this.selAllCheckBoxObj.setValue(isSelAll);
			}
		}
	...
	Table.prototype._updateEvent = function(state){
		...
		if (this._ops.hasCheckBox)
		{
			...
			if (this._ops.hasHead == true)
			{
				this._ops.hasAllCheckBox && this.selAllCheckBoxObj.setValue(isSelAll);
			}
		}
	}
	...
	Table.prototype._btnHandle = function(listObj, dataObj, td, index){
...
*/
if (!Array.prototype.find && (!isIE || isIENormal)) {
