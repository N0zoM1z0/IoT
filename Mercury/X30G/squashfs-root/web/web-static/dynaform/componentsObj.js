var componentObj = function(){
	...
	this.confirmDialog = new Dialog({
		...
		closeIconHandle: function(){
			confirmDialog.hide(function(){
				if(typeof confirmDialog.iconCloseCallBack == "function") {
					confirmDialog.iconCloseCallBack();
					return;
				}
	...
	this.confirmDialog.iconCloseCallBack = null;
	...
	this.confirmDialog.show = function(option){
		...
		if (option != undefined && $.isPlainObject(option))
		{
			...
			if (option.iconCloseCallBack != undefined)
			{
				self.iconCloseCallBack = option.iconCloseCallBack;
			}
			else
			{
				self.iconCloseCallBack = null;
			}
		}
	...
	this.cloudPageDialog.show = function(){
		$("#Dialog").addClass("dialogAbsolute");
		Dialog.prototype.show.call(this);
	}
	this.cloudPageDialog.hide = function(func){
		$("#Dialog").removeClass("dialogAbsolute");
		Dialog.prototype.hide.call(this,func);
	}
	...
	this.cloudAppDialog = new Dialog({
	...
	this.cloudAppDialog.bodyClickHandle = function(e){
		detachEvnt(document.body, "click", cloudAppDialog.bodyClickHandle);

		cloudAppDialog.hide(function(){
			//document.body.style.overflow = "auto";
		});
	}
	this.cloudAppDialog.show = function(event){
		var self = this;
	...
	this.initMeshDialog = function(){
		...
		this.meshTopoDialog.show = function(){
			$("#Dialog").addClass("dialogAbsolute");
			Dialog.prototype.show.call(this);
		}
		this.meshTopoDialog.hide = function(func){
			$("#Dialog").removeClass("dialogAbsolute");
			Dialog.prototype.hide.call(this,func);
		}
		...
		this.meshAddDialog.hide = function(func){
			$("#Dialog").removeClass("dialogAbsolute");
			Dialog.prototype.hide.call(this,func);
		}
		...
		this.meshDescDialog = new Dialog({
			title: btn.meshFunction,
			size: Dialog.SIZE.LARGE,
			type: Dialog.TYPE.CUSTOM,
			isConstant: true,
			content: '<div class="meshDescTitle"></div>' +
					 '<div class="meshDescContent" id="meshDescContent"></div>',
			className: "meshDescPage",
			renderCallBack: function(){
				var self = this;

				loadDialogRenderPage("meshDescProcess.htm", "meshDescContent", function(){
					new Button({
						type:Button.TYPE.PRIMARY,
						text:btn.ok,
						id: "meshDescReadySub",
	...
	this.dfsDialog = new Dialog({
		hasTitle: false,
		size: Dialog.SIZE.SMALL,
		hasCloseIcon: false,
		type: Dialog.TYPE.CUSTOM,
		isConstant: true,
		content: "<div class='dfsCon'>\
					<p class='dfsText'>遵照国家法律法规，路由器正在做退避雷达信号探测，<br />Wi-Fi信号要 <span id='DFScountdown'></span><span class='dfsSecond'>秒</span> 后才能上线，请稍候...</p>\
				</div>"
	});
	this.dfsDialog.show = function(countdown, callback)
	{
		var self = this;
	...
	this.phoneDfsDialog = new Dialog({
		hasTitle: false,
		size: Dialog.SIZE.PHONE,
		hasCloseIcon: false,
		type: Dialog.TYPE.CUSTOM,
		isConstant: true,
		content: "<div class='dfsCon'>\
					<p class='dfsText'>遵照国家法律法规，路由器正在做退避雷达信号探测，<br />Wi-Fi信号要 <span id='phoneDFScountdown'></span> 后才能上线，请稍候...</p>\
				</div>"
	});
	this.phoneDfsDialog.show = function(countdown, callback)
	{
		var self = this;
		var countdownHandle = null;
		...
		if (typeof countdown != "undefined")
		{
			$("#phoneDFScountdown").html(countdown + "秒");
			countdownHandle = setInterval(function(){
				countdown--;
				$("#phoneDFScountdown").html(countdown + "秒");
				if (countdown <= 0)
	...
	this.phoneConfirmDialog = new Dialog({
		hasTitle: false,
		size: Dialog.SIZE.PHONE,
		hasCloseIcon: false,
		type: Dialog.TYPE.CUSTOM,
		isConstant: true,
		content: '<div class="phoneConfirmCon"></div>',
		className: 'phoneConfirmDialog',
		bottom: [
			{
				type:component.Button.TYPE.SECONDARY,
				text:btn.cancelTip,
				id:'phoneConfirmCancel',
				onClick:function(){
					phoneConfirmDialog.hide(function(){
						typeof phoneConfirmDialog.clickCallback == "function" && phoneConfirmDialog.clickCallback(false);
					});
				}
			},
			{
				type:component.Button.TYPE.PRIMARY,
				text:btn.confirmTip,
				id:'phoneConfirmSubmit',
				onClick:function(){
					phoneConfirmDialog.hide(function(){
						typeof phoneConfirmDialog.clickCallback == "function" && phoneConfirmDialog.clickCallback(true);
					});
				}
			}
		]
	});
	this.phoneConfirmDialog.show = function(option)
	{
		var self = this;
		...
		if (option != undefined && $.isPlainObject(option))
		{
			...
			if (option.content != undefined && typeof option.content == "string")
			{
				$(self.container).find("div.dialogContent .phoneConfirmCon").html(option.content);
			}
