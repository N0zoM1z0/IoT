...
ppp_generic_setup() {
	...
	pppoe_ipv6_only=$(uci -q get network.wan.pppoe_ipv6_only)
	if [ "$pppoe_ipv6_only" = "1" ]; then
		proto_run_command "$config" /usr/sbin/pppd \
			nodetach ipparam "$config" \
			ifname "$pppname" \
			${localip:+$localip:} \
...
proto_pppoe_setup() {
	...
	lcp_echo_enable=$(uci -q get network.wan.lcp_echo_enable)
	sed -i '/lcp-echo-failure 5/d' /etc/ppp/options
	sed -i '/lcp-echo-interval 30/d' /etc/ppp/options
	if [ $lcp_echo_enable -eq 1 ]; then
		echo "lcp-echo-failure 5" >> /etc/ppp/options
		echo "lcp-echo-interval 30" >> /etc/ppp/options
	fi

	mtu=$(uci -q get network.wan.pmtu)
...
proto_pptp_setup() {
	...
	sed -i '/mppe required,stateless/d' /etc/ppp/options.pptp
	sed -i '/nomppe/d' /etc/ppp/options.pptp
	mppe=$(uci -q get network.vpn.mppe)
	if [ $mppe -eq 1 ]; then
	...
	sed -i '/mppc/d' /etc/ppp/options.pptp
	sed -i '/nomppc/d' /etc/ppp/options.pptp
	mppc=$(uci -q get network.vpn.mppc)
	if [ $mppc -eq 1 ]; then
		echo "mppc" >> /etc/ppp/options.pptp
	else
		echo "nomppc" >> /etc/ppp/options.pptp
	fi

	sed -i '/mtu/d' /etc/ppp/options.pptp
	mtu=$(uci -q get network.vpn.mtu)
	echo "mtu ${mtu}" >> /etc/ppp/options.pptp
}
