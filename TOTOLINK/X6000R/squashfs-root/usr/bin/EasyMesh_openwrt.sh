...
prepare_ifame_band()
{
	...
	wisp_band=`uci -q get wireless.apcli.device`
	card_ext_ifname=`cat /etc/wireless/l1profile.dat | grep INDEX${card_idx}_ext_ifname |awk -F "[=;]" '{ print $2 }'`
	echo $card_ext_ifname
	card_profile_path=`cat /etc/wireless/l1profile.dat | grep INDEX${card_idx}_profile_path |awk -F "[=;]" '{ print $2 }'`
	echo $card_profile_path
	...
	then
		...
		brctl delif $bridge_name $card_apcli_ifname"0"
		if [ $opmode_custom != "wisp" -o $wisp_band != "radio_2g" ]; then
			brctl addif $bridge_name $card_apcli_ifname"0"
		fi
	fi
	echo $if_list
	...
	then
		...
		brctl delif $bridge_name $card_apcli_ifname"0"
		if [ $opmode_custom != "wisp" -o $wisp_band != "radio_5g" ]; then
			brctl addif $bridge_name $card_apcli_ifname"0"
		fi
	fi
	echo $if_list
...
is_radio_up()
{
	...
	do
		...
		then
			...
			dbdc=`cat /etc/wireless/l1profile.dat | grep INDEX${card_idx}_main_ifname= | awk -F ";" '{ print $2 }'`

			if [ -z "$dbdc" ]
			then
				echo "non dbdc"
				card_main_ifname3=`cat /etc/wireless/l1profile.dat | grep INDEX${card_idx}_main_ifname= |
							awk -F "[=]" '{ print $2 }'`
				echo $card_main_ifname3
				while :
				do
					iw3=`iwconfig $card_main_ifname3`
					if [[ -n "$iw3" ]]
					then
						echo "non dbdc interface is up"
						break
					else
						echo "non dbdc interface not up"
						break
					fi
				done
			else
				echo "dbdc detected"
				card_main_ifname1=`cat /etc/wireless/l1profile.dat | grep INDEX${card_idx}_main_ifname= |
							awk -F "[=;]" '{ print $2 }'`
				card_main_ifname2=`cat /etc/wireless/l1profile.dat | grep INDEX${card_idx}_main_ifname= |
							awk -F "[=;]" '{ print $3 }'`
				echo $card_main_ifname1
				echo $card_main_ifname2
				while :
				do
					iw1=`iwconfig $card_main_ifname1`
					iw2=`iwconfig $card_main_ifname2`
					if [[ -n "$iw1" && -n "$iw2" ]]
					then
						echo "dbdc interface is up"
						break
					else
						echo "dbdc interface not up"
						break
					fi
				done
			fi
...
prepare_1905_config()
{
		...
		wanstr='wan='
		#br0_mac=$(cat /sys/class/net/br-lan/address)
		br0_mac=`cs mac r 2g | awk '{print $2}'`
...
LowRSSIAPSteerEdge_root=35
force_roam_rssi_th=-65
MinRssiIncTh_Root=5
MinRssiIncTh_RE=10
MinRssiIncTh_Peer=10
CUOverloadTh_2G=70
CUOverloadTh_5G_L=100
CUOverloadTh_5G_H=100
CUOverloadTh_6G=100
MetricPolicyChUtilThres_24G=65
MetricPolicyChUtilThres_5GL=70
MetricPolicyChUtilThres_5GH=70
MetricPolicyChUtilThres_6G=80
ChPlanningChUtilThresh_24G=65
ChPlanningChUtilThresh_5GL=75
...
RSSISteeringEdge_DG=20
RSSISteeringEdge_UG=20
MCSCrossingThreshold_UG=50000
RSSICrossingThreshold_UG=20
MCSCrossingThreshold_DG=20
RSSICrossingThreshold_DG=50000
" > /etc/mapd_strng.conf
	fi
}
...
prepare_mapd_config()
{
	...
	sed -i "s/bss_config_priority=.*/bss_config_priority=${bh_priority_list}/g" /etc/map/1905d.cfg
	prepare_mapd_strng_configs
}
...
#	ModifySwitchReg
	sleep 2
	killall mesh_sync 2>&1 >/dev/null
	sleep 1
	mesh_sync sync 2>&1 >/dev/null &
	echo -e "----- ${RED}MAP DEVICE STARTED${NC} -----"
}
datconf -f /tmp/cste/temp_status set wifi_reload 1
clean
sync
is_radio_up
sync
...
datconf -f /tmp/cste/temp_status set wifi_reload 0
