...
{
	var str = "";
	str = "<TR><TD class=CONTENT_TITLE_L colspan=2>"+__T(wz.title);
	str += "</TD></TR>";
	document.write(str);
...
{
	var str=""
	var img1_style="",img2_style="",img3_style="",img4_style="";
	var item1_style="",item2_style="",item3_style="",item4_style="";
	img1_style=img2_style=img3_style=img4_style="style=display:none";
	item1_style=item2_style=item3_style=item4_style="ITEMS";
	str = "<TR height=30px bgolor=blue valign=top>";
	if ( parseInt(pass_item,10) >= 1 ) img1_style = "";
	if ( parseInt(pass_item,10) >= 2 ) img2_style = "";
	if ( parseInt(pass_item,10) >= 3 ) img3_style = "";
	if ( parseInt(pass_item,10) >= 4 ) img4_style = "";
	if ( sel_item == "1" ) item1_style = "ITEMS_SEL";
	if ( sel_item == "2" ) item2_style = "ITEMS_SEL";
	if ( sel_item == "3" ) item3_style = "ITEMS_SEL";
	if ( sel_item == "4" ) item4_style = "ITEMS_SEL";
	if ( parseInt(pass_item,10) >= 1 ) item1_style = "ITEMS_CHK";
	if ( parseInt(pass_item,10) >= 2 ) item2_style = "ITEMS_CHK";
	if ( parseInt(pass_item,10) >= 3 ) item3_style = "ITEMS_CHK";
	if ( parseInt(pass_item,10) >= 4 ) item4_style = "ITEMS_CHK";

//	if ( current_model_name == "RV130W" ) 
//		var name_list = new Array(__T(wz.item1),__T(wz.item2),__T(wz.item3),__T(wz.item4w));
//	else
		var name_list = new Array(__T(wz.item1),__T(wz.item2),__T(wz.item3),__T(wz.item4));
	for(var i=0; i<4; i++)
	{
		str += "<TD align=center><TABLE><TR><TD id=item"+(i+1)+" "+eval("img"+(i+1)+"_style")+"><img src=../image/acknowledge.png></TD><TD class="+eval("item"+(i+1)+"_style")+">"+name_list[i]+"</TD></TR></TABLE></TD>"; 
	}
        str += "</TR>"
	document.write(str);
}
...
function save_data(objname,objval,flg)
{
	...
	if ( level )
		var data = top.document.getElementById("wizard_var").value;
	else
	...
	{
		if ( level )
			top.document.getElementById("wizard_var").value += objname+"="+tmpcnt+":"+objval;
		else
		...
		{
			if ( level )
				top.document.getElementById("def_wizard_var").value += objname+"="+tmpcnt+":"+objval;
			else
				parent.document.getElementById("def_wizard_var").value += objname+"="+tmpcnt+":"+objval;
		}
	}else{
		...
		if ( level )
			top.document.getElementById("wizard_var").value = data2;
		else
			parent.document.getElementById("wizard_var").value = data2;
		if ( initflg == 1 )  
		{
			if ( level )
				top.document.getElementById("def_wizard_var").value = data2;
			else
				parent.document.getElementById("def_wizard_var").value = data2;
		}
	}
}
...
function w_valid_mask(obj,flag,span_id,direct)
{
	...
	for(var i=0; i<4; i++)
	{
		...
		{
			m[i] = document.getElementById(obj+"_"+i).value;
			if ( !chk_range(obj+"_"+i,document.getElementById(obj+"_"+i).value,ip_range[0],ip_range[1],10) )
				errflg=1;
		}
	}
	if( m[0] == "0" && m[1] == "0" && m[2] == "0" && m[3] == "0" ){
...
function pptp_l2tp_valid(F)
{
        ...
        if ( !w_valid_ip("IP","ip",VALID_IP_RULE7,"msg_ip",ZERO_NO,"down") ) errflg=1;
        if ( !w_valid_ip("IP","gw",VALID_IP_RULE7,"msg_gw",ZERO_NO,"down") )errflg=1;
        if ( !w_valid_ip("IP","sip",VALID_IP_RULE7,"msg_sip",ZERO_NO,"down") ) errflg=1;
