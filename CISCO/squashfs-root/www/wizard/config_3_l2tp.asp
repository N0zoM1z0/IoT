...
function init()
{
	...
	var l2tp_name = parse_data("ppp_username");
	var l2tp_pwd = parse_data("ppp_passwd");
	var l2tp_ip = parse_data("wan_ipaddr").split(".");
	var l2tp_mask = parse_data("wan_netmask").split(".");
	var l2tp_gw = parse_data("wan_gateway").split(".");
	var l2tp_sip = parse_data("l2tp_server_ip").split(".");
	F.accname.value = l2tp_name;
	F.pwd.value = l2tp_pwd;
	for(var i=0; i<l2tp_ip.length; i++)
		eval("F.ip_"+i).value = l2tp_ip[i];
	for(var i=0; i<l2tp_mask.length; i++)
		eval("F.mask_"+i).value = l2tp_mask[i];
	for(var i=0; i<l2tp_gw.length; i++)
		eval("F.gw_"+i).value = l2tp_gw[i];
	for(var i=0; i<l2tp_sip.length; i++)
		eval("F.sip_"+i).value = l2tp_sip[i];
}
...
<TABLE class=CONTENT_TABLE cellspacing=0>
    ...
    <TR><TD class=DATA_TITLE2 colspan=4><script>Capture(wz.setl2tp);</script></TD></TR>
    <TR><TD colspan=4 valign=top>
    <TABLE><TR><TD valign=top class=DATA_ITEM>
    <script>Capture(wz.setl2tpmsg);</script>
    </TD></TR>
    ...
    <TABLE width=100% height=100%>
	   <col width=17%><col width=33%><col width=17%><col width=33%>
           <TR><TD><script>Capture(wz.accountname);</script></TD>
           <TD><span id=msg_accname></span><input size=19 name=accname id=accname maxlength=63></TD>
	   ...
	   <input size=1 maxlength=3 name=ip_0 id=ip_0>
           ...
           <input size=1 maxlength=3 name=ip_3 id=ip_3>
           </TD>
           </TR>
           <TR><TD><script>Capture(share.password);</script></TD><TD><span id=msg_pwd></span><input size=19 name=pwd type=password id=pwd maxlength=63></TD>
	   ...
	   <input size=1 maxlength=3 name=mask_0 id=mask_0>
           ...
           <input size=1 maxlength=3 name=mask_3 id=mask_3>
	   </div>
           </TD>
           </TR>
           <TR><TD><script>Capture(wz.confirmpwd)</script></TD><TD><input size=19 name=cf_pwd id=cf_pwd type=password maxlength=63></TD>
	   ...
	   <span id=msg_gw></span>
           ...
           <input size=1 maxlength=3 name=gw_3 id=gw_3>
           </TD> 
           </TR>
           <TR><TD colspan=2></TD><TD><script>Capture(wz.serveripaddr);</script></TD><TD>
	   ...
	   <input size=1 maxlength=3 name=sip_0 id=sip_0>
           ...
           <input size=1 maxlength=3 name=sip_3 id=sip_3>
           </TD>
           </TR>
    </TABLE>
    </TD></TR>
