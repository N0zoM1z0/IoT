...
var delay_time=0;
function reload(){
	goto_page("status_rac_ipsec.asp");
}
...
function DoConnect(i,idx, conn)
{
	...
	F.submit_button.value="status_rac_ipsec";
...
<TABLE class=CONTENT_TABLE>
  ...
  <TR><TD colspan=2>
		...
		<TR><TD colspan=11 class=TABLETITLE><script>Capture(vpn.stitle);</script></TR>	
		<TR height=20px>
		...
		<TD rowspan=2 class=TABLECONTENT_TD style="border-right:1px solid #8499A2;border-bottom:1px solid #8499A2">AH</TD>
		<TD rowspan=2 class=TABLECONTENT_TD style="border-right:1px solid #8499A2;border-bottom:1px solid #8499A2">ESP</TD>
		...
		<script>
			...
			if ( ipsec_st.length == 0 ) 
			{
	                        ...
	                        document.write("<TD class='TABLECONTENT_TD' colspan=11>"+__T(msg.nodata)+"</TD>");
			}
			...
			for(var i=0; i<ipsec_st.length; i++)
			{
                                ...
                                document.write("<TR class="+tstyle+" id='d"+i+"'>");
				document.write("<TD class=TABLECONTENT_TD>"+cy_speccode_decode(ipsec_st[i][0])+"</TD>");
				document.write("<TD class=TABLECONTENT_TD>"+ipsec_st[i][1]+"</TD>");
...
//				document.write("<TD class=TABLECONTENT_TD_VM>"+ipsec_st[i][5]+"</TD>");
				
				for(var j=5; j<(ipsec_st[i].length-1); j++){
				{
					...
					if ( nv_unit == "1" ) 
					{
						...
						data = formatNumByComma(ipsec_st[i][j],lang);
						document.write(data);
						document.write("</TD>");
					}
				}
				document.write("<TD class=TABLECONTENT_TD>"+ipsec_st[i][j]+"</TD>");
