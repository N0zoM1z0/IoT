...
{
	var ch="";
	var num=0,schar=0,bchar=0,other=0;
	var now_class=0;
	...
	for(var i=0; i<val.length; i++)
	{
	    	ch = val.charCodeAt(i);
		if( ch >= 48 && ch <= 57 ) num++;
		else if( ch >= 65 && ch <= 90  ) bchar++;
		else if( ch >= 97 && ch <= 122 ) schar++;
		else other++; 
	}
	
	if ( num>0 ) now_class ++;
	if ( schar>0 ) now_class ++;
	if ( bchar>0 ) now_class ++;
	if ( other>0 )  now_class++;
	if ( val.length == 0 ) return 0;
	if ( pwd_class > now_class || len > val.length ) return 4;
	var tmp = parseInt(64-len,10)/10; 
	var score = parseInt(val.length/tmp,10);
	if ( score == 0 ) score = 5;
	else score+=4;
	return score;
}
var en_guest = "<% nvram_get("en_guest"); %>";
...
function init()
{
  ...
  if(window.top != window.self) {
  	window.top.location = "/login.asp"+get_url_key(1);
  }
...
<body onload=init()> 
<FORM id=frm name=login method=<% get_http_method(); %> action="login.cgi" onKeyDown=chk_keypress(event) autocomplete=off>
...
<TABLE cellspacing=0 class=MAINTABLE>
  	...
  	<TD class=APPNAME ><script>Capture(share.router);</script></TD>
