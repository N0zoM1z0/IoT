...
var support_3g = 0;
<% support_match("WWAN_SUPPORT","1", "support_3g=1"); %>

var PASS=1;
var FAIL=0;
var Browser = CheckBrowser();
var current_model_name = "<% nvram_get("model_name"); %>";
...
var ap_mode = "<% nvram_get("access_point_enable"); %>";
...
function get_url_filename(flag)
{
	...
	if(flag == 1) 
	{
		...
		{
			if ( parent.frames['content_area'].location.href.indexOf("change_password.asp") != -1 )
				NOWPATH = "change_password.asp";	
		}	
		if( ( NOWPATH.indexOf("change_password.asp") != -1 ))
			ret = 1;
		
		if ( NOWPATH.indexOf("default.asp") != -1 ) ret=0;
	}
	else //login.cg
	{
		if( NOWPATH.indexOf("default.asp") != -1 ) 
			ret = 1;
	}
...
function bt(x,st)
{
        ...
        if ( st == "move" )
        {
               obj.style.background="url('/image/BT_Hover.jpg')";
               obj.style.border="1px solid #1fa0d5";
               obj.style.borderwidth="thick thin";
        }
	else if ( st == "out" )
	{
              obj.style.background="url('/image/BT_Normal.jpg')";
              obj.style.border="1px solid #53636a";
              obj.style.borderwidth="thick thin";
        }
	else if ( st == "down" )
	{
               obj.style.background="url('/image/BT_Press.jpg')";
               obj.style.border="1px solid #1fa0d5";
               obj.style.borderwidth="thick thin";
        }else if ( st == "disabled" ) 
	{
               obj.style.background="url('/image/BT_Disabled.jpg')";
	...
	if ( window.event )
	{
		...
		if ( obj_pos < 0 ) 
		{
			...
			if ( obj_pos < 200 )
      			{	
				if ( Browser == "IE" ) 
					s.style.pixelLeft = e.x+document.body.scrollLeft-obj_pos-130;
				else
				        s.style.left = e.pageX-obj_pos-150+"px";//
      			}
			else
		        {
				if ( Browser == "IE" ) 
		...
		if ( obj_pos < 0 ) 
			...
			if ( obj_pos < 200 ) 
			    s.style.left = e.pageX-obj_pos-130+"px";
			else
		            s.style.left = e.pageX-15+"px";
		}
	}
...
function get_result_msg(filename,flg)
{
	...
	if ( flg == 0 )
		...
		if ( typeof(args[2])!='undefined' )
		{
			...
			//else if ( filename == "vpn_client.asp" || filename == "backup.asp" ) 
			else if ( filename == "vpn_client.asp") 
				return __T(msg.invalidcfg);
			else if( filename == "backup.asp")
			{	
				if (flg == 1)
					return __T(msg.invalidcfg);
				else if(flg == 2)
			...
			else if ( filename == "ssl_cert.asp" && flg == 2 )
				return __T(vpn.certsubjecterr); 
			else if ( filename == "man_cerificate.asp" || filename == "ssl_cert.asp" ) 
				return __T(msg.invalidcert);
			else if ( filename == "users.asp" ) 
				return __T(msg.invaliduser);
			else if ( filename == "Wireless_welcome_edit.asp" ) 
				return __T(msg.invalidimage);
			else if ( filename == "authap.asp" && flg == 1 ) 
	...
	if ( output == O_MSG )
	{
		...
		if ( args[1] == WARNING ) 
		{
			...
			if ( args[6] != "" && args[6] == "wizard" ) 
			{
				...
				else if ( args[7] != "" && args[7] =="pptp"){
					parent.document.getElementById("alert_bt").innerHTML= draw_object(O_VAR,BT,okbt,"a1",okstyle,"parent.content_area."+args[4]) + draw_object(O_VAR,BT,cabt,"a2",castyle,"parent.content_area."+args[5]) + draw_object(O_VAR,BT,cabt2,"a3",castyle2,"parent.content_area."+args[6]);
...
var GET_NUM=5;
var GET_NAME=6;
var CHK_VLAN_RANGE=7;
var GET_WPS_METHOD3=8;
var GET_LINK=9;
var DEF_RANGE=10;
var HINT_MSG=11;
var ALL_CHK_RANGE=12; 
var DATE_TIME1=13; //12/13/10 06:00PM
var DATE_TIME2=14; //0 Days, 1 HOurs, 6 Minutes, 57 Seconds
var DATE_TIME3=15; //2010-12-13 19:05:14
var DATE_TIME4=16; //21:11 PM
var RANGE=17;
var BSD=18;  //for bsd usage

function get_spell_words()
{
	var args = new Array;
	for(var i=0; i<10; i++)
	{
	...
	var type = parseInt(args[0],10);
	var new_msg;
	if ( type == RANGE ){
		new_msg = range_rule2; 
       		new_msg = new_msg.replace(/<from>/,args[1]);
       		new_msg = new_msg.replace(/<to>/,args[2]);
	}else if ( type == CHK_VLAN_RANGE ){
		new_msg = vlan_range_rule; 
       		new_msg = new_msg.replace(/<range1>/,args[1]);
       		new_msg = new_msg.replace(/<range2>/,args[2]);
	}else if ( type == CHK_RANGE ){
		new_msg = range_rule; 
       		new_msg = new_msg.replace(/<range1>/,args[1]);
       		new_msg = new_msg.replace(/<range2>/,args[2]);
	}else if ( type == CHK_MAX_ENTRY ){
		new_msg = max_rule; 
       		new_msg = new_msg.replace(/<name>/,args[2]);
       		new_msg = new_msg.replace(/<max>/,args[1]);
	}else if ( type == PPP_DEMAND_1 || type == PPP_DEMAND_2 ){
		if ( type == PPP_DEMAND_1 ) 
			new_msg = connect_on_demand_rule; 
		else
			new_msg = keep_alive_rule; 
       		new_msg = new_msg.replace(/<time>/,args[1]);
	}else if ( type == GET_NUM ){
		new_msg = args[1];
       		new_msg = new_msg.replace(/<num>/,args[2]);
	}else if ( type == GET_NAME ){
		new_msg = args[1];
       		new_msg = new_msg.replace(/<name>/,args[2]);
	}else if ( type == GET_WPS_METHOD3 ){
		new_msg = wps_method3_rule; 
       		new_msg = new_msg.replace(/<pin_num>/,args[1]);
	}else if ( type == GET_LINK ){
		new_msg = args[1];
		new_msg = new_msg.replace(/<link>/,args[2]);
	}else if ( type == DEF_RANGE ){
		new_msg = range_def_rule;
		new_msg = new_msg.replace(/<from>/,args[1]);
		new_msg = new_msg.replace(/<to>/,args[2]);
		new_msg = new_msg.replace(/<val>/,args[3]);
	}else if ( type == HINT_MSG ){ 
		new_msg = hint_rule;
		new_msg = new_msg.replace(/<val>/,args[1]);
	}else if ( type == ALL_CHK_RANGE ){
		new_msg = args[1];
		new_msg = new_msg.replace(/<from>/,args[2]);
		new_msg = new_msg.replace(/<to>/,args[3]);
	}else if ( type == DATE_TIME1 ){
		new_msg = datetime_rule; 
		new_msg = new_msg.replace(/<mon>/,args[1]);
		new_msg = new_msg.replace(/<day>/,args[2]);
		new_msg = new_msg.replace(/<year>/,args[3]);
		new_msg = new_msg.replace(/<hour>/,args[4]);
		new_msg = new_msg.replace(/<min>/,args[5]);
		new_msg = new_msg.replace(/<pm>/,args[6]);
	}else if ( type == DATE_TIME2 ){
		new_msg = datetime1_rule; 
		new_msg = new_msg.replace(/<day>/,args[1]);
		new_msg = new_msg.replace(/<hour>/,args[2]);
		new_msg = new_msg.replace(/<min>/,args[3]);
		new_msg = new_msg.replace(/<sec>/,args[4]);
	}else if ( type == DATE_TIME3 ){
	        new_msg = datetime2_rule;
                new_msg = new_msg.replace(/<year>/,args[1]);
                new_msg = new_msg.replace(/<mon>/,args[2]);
                new_msg = new_msg.replace(/<day>/,args[3]);
                new_msg = new_msg.replace(/<hour>/,args[4]);
                new_msg = new_msg.replace(/<min>/,args[5]);
                new_msg = new_msg.replace(/<sec>/,args[6]);
	}else if ( type == DATE_TIME4 ){
	        new_msg = datetime3_rule;
                new_msg = new_msg.replace(/<hour>/,args[1]);
                new_msg = new_msg.replace(/<min>/,args[2]);
                new_msg = new_msg.replace(/<pm>/,args[3]);
	}else if ( type == BSD){
		new_msg = args[1];
		new_msg = new_msg.replace(/<value1>/,args[2]);
		new_msg = new_msg.replace(/<value2>/,args[3]);
		new_msg = new_msg.replace(/<value3>/,args[4]);
	}
	return new_msg;
}
