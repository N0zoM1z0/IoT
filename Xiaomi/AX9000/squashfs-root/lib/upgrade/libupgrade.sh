...
do_flash_rpm() {
	local package=$1
	local segment_name="rpm.mbn.padded"

	do_flash_failsafe_partition ${package} ${segment_name} "0:RPM"
	if [ $? -eq 0 ]; then
		klogger "Done"
	else
		klogger "Error"
...
do_flash_firmware_bak() {
        local package=$1
        local segment_name="root-bak.ubi"

	local flag=`bootinfo | grep CHANNEL |awk -F \' '{print $2}'`
	local bak_mtd=$(grep '"mifw_bak"' /proc/mtd | awk -F: '{print substr($1,4)}')
	if [ -n "$bak_mtd" -a "$flag" == "stable" ]; then
        	do_flash_failsafe_partition ${package} ${segment_name} "mifw_bak"
        	if [ $? -eq 0 ]; then
                	klogger "Done"
        	else
                	klogger "Error"
