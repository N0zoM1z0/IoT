...
mimesh_re_assoc_check()
{
	...
	local iface_5g_bh=$(uci -q get misc.backhauls.backhaul_${bh_tag}_sta_iface)
	[ -z "$iface_5g_bh" ] && iface_5g_bh="wl21"
...
mimesh_cap_bh_check()
{
	local bh_tag=$(uci -q get misc.backhauls.backhaul)
	[ -z "$bh_tag" ] && bh_tag="$BHTAG"
	local iface_5g_bh=$(uci -q get misc.backhauls.backhaul_${bh_tag}_ap_iface)
	[ -z "$iface_5g_bh" ] && iface_5g_bh="wl7"
