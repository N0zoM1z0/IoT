...
__init_wifi_cap()
{
	...
	#[ "$WIFI2_EXIST" = "1" ] && iface_idx_start=$((iface_idx_start+1)) #3
	ifconfig wifi3 >/dev/null 2>&1 && export WIFI3_EXIST=1 || export WIFI3_EXIST=0
	#[ "$WIFI3_EXIST" = "1" ] && iface_idx_start=$((iface_idx_start+1)) #4
	...
	local ifname_game=$(uci -q get misc.wireless.iface_game_ifname)
	local iface_game_index=$(uci show wireless |grep -w "ifname=\'$ifname_game\'" | cut -d '[' -f2 | cut -d ']' -f1)
	
	# wifi ap ifaces
	iface_list="0 1 2"
	for ii in ${iface_list}; do
		if [ "$bsd" -eq 0 -o "$bsd" -eq 2 ]; then
:'
...
'
			if [ "$ii" -eq "$iface_5g_index" ]; then
				...
				main_mgmt=$mgmt_5g
				main_pswd=$pswd_5g
			elif [ "$ii" -eq "$iface_2g_index" ]; then
				main_ssid=$ssid_2g
				main_mgmt=$mgmt_2g
				main_pswd=$pswd_2g
			elif [ "$ii" -eq "$iface_game_index" ]; then
				main_ssid=$ssid_game
				main_mgmt=$mgmt_game
				main_pswd=$pswd_game
			fi
		else
			if [ "$ii" -ne "$iface_game_index" ]; then
				main_ssid=$whc_ssid
				main_mgmt=$whc_mgmt
				main_pswd=$whc_pswd
			else
...
EOF
	...
	local bh_ifname="`uci get misc.backhauls.backhaul_${bh_tag}_ap_iface`"
	ii=$iface_idx_start

	local bh_radio_device=$(uci -q get misc.wireless.if_${bh_tag})
	[ "$bh_tag" = "$BHTAG_SEC" ] && {
		local BHDEV=$(echo $bh_tag | sed 'y/aegm/AEGM/')
		bh_radio_device=$(uci -q get misc.wireless.if_${BHDEV})
	}
	local bh_radio_ifname=$(uci -q get misc.wireless.ifname_${bh_tag})
	local bh_radio_iface=$(uci show wireless | grep -w "ifname=\'$bh_radio_ifname\'" | awk -F"." '{print $2}')
	local lanmac=$(ifconfig br-lan | grep HWaddr | awk '{print $5}')

	uci -q batch <<-EOF >/dev/null
add wireless wifi-iface
set wireless.@wifi-iface[$ii].device="$bh_radio_device"
...
'
	# set bsd
	[ "$bsd" -gt 0 ] && {
		uci -q set wireless.@wifi-iface[0].bsd="$bsd"
		uci -q set wireless.@wifi-iface[1].bsd="$bsd"
		uci -q set wireless.@wifi-iface[2].bsd="$bsd"
	}
	uci -q set wireless.$bh_radio_iface.miwifi_mesh='0'

	uci -q set wireless.$bh_radio_device.CSwOpts='0x31'
	...
	[ "$WIFI3_EXIST" = "1" ] && {
		uci -q delete wireless.wifi3.whc_uninit
	}
...
__init_network_re() {
	...
	# no wan on son re, so delete wan/wan6/vpn
	uci -q batch <<-EOF >/dev/null
...
EOF
	...
	#stop no-use service for RE
	ifdown vpn 2>/dev/null
	[ -f /usr/sbin/vasinfo_fw.sh ] && /usr/sbin/vasinfo_fw.sh off 2>/dev/null
...
__init_wifi_re()
{
	...
	uci -q set wireless.$device_game.CSwOpts='0x31'
...
EOF
	...
	[ -z "$bh_caps" ] && bh_caps="$BHTAG"

	for bh_tag in $bh_caps; do
		if [ "$bh_tag" != "$BHTAG" ] && [ "$bh_tag" != "$BHTAG_SEC" ]; then
			continue
		fi
		local bh_radio_device=$(uci -q get misc.wireless.if_${bh_tag})
		[ "$bh_tag" = "$BHTAG_SEC" ] && {
...
EOF
		# 5G radio
		[ "$bh_tag" != "$BHTAG" ] && uci -q set wireless.@wifi-iface[$ii].channel_block_list='36,40,44,48,52,56,60,64,165'
...
'
		...
		local bh_sta_ifname=$(uci get misc.backhauls.backhaul_${bh_tag}_sta_iface)
...
EOF
	...
	uci -q set wireless.@wifi-iface[$iface_game_index].miwifi_mesh='0'
	uci -q set wireless.@wifi-iface[$iface_5g_index].miwifi_mesh='0'
...
__mimesh_check_wireless()
{
	...
	[ "$bh_tag" = "$BHTAG_SEC" ] && {
		...
		ifname_5g=$(uci -q get misc.wireless.ifname_${BHDEV})
	}
	local backhaul_5g=$(uci -q get misc.backhauls.backhaul_${bh_tag}_ap_iface)
...
mimesh_init()
{
	...
	[ "$bsd" -eq 0 -o "$bsd" -eq 2 ] && key_list="whc_role ssid_2g mgmt_2g pswd_2g ssid_5g mgmt_5g pswd_5g ssid_game mgmt_game pswd_game" || key_list="whc_role whc_ssid whc_pswd whc_mgmt ssid_game mgmt_game pswd_game"
	...
	if [ "$bsd" -eq 0 -o "$bsd" -eq 2 ]; then
        [ -z "$ssid_2g" ] && ssid_2g="!@Mi-son" || ssid_2g="$(base64_dec $ssid_2g)"
		[ -z "$mgmt_2g" ] && mgmt_2g="mixed-psk"
        [ -z "$pswd_2g" ] && mgmt_2g="none" || pswd_2g="$(base64_dec $pswd_2g)"
        [ -z "$ssid_5g" ] && ssid_5g="!@Mi-son_5G" || ssid_5g="$(base64_dec $ssid_5g)"
		[ -z "$mgmt_5g" ] && mgmt_5g="mixed-psk"
        [ -z "$pswd_5g" ] && mgmt_5g="none" || pswd_5g="$(base64_dec $pswd_5g)"
        [ -z "$ssid_game" ] && ssid_game="!@Mi-son_5G_Game" || ssid_game="$(base64_dec $ssid_game)"
		[ -z "$mgmt_game" ] && mgmt_game="mixed-psk"
        [ -z "$pswd_game" ] && mgmt_game="none" || pswd_game="$(base64_dec $pswd_game)"
		...
		MIMESH_LOGI " keys:<$whc_role>,<$bsd>,<$ssid_2g>,<$pswd_2g>,<$mgmt_2g>,<$ssid_5g>,<$pswd_5g>,<$mgmt_5g>,<$ssid_game>,<$pswd_game>,<$mgmt_game>,<$bh_ssid>,<$bh_pswd>,<$bh_mgmt>"
		MIMESH_LOGI " keys:<$bh_macnum_2g>,<$bh_maclist_2g>,<$bh_macnum_5g>,<$bh_maclist_5g>"

	else
        [ -z "$whc_ssid" ] && whc_ssid="!@Mi-son" || whc_ssid="$(base64_dec $whc_ssid)"
		[ -z "$whc_mgmt" ] && whc_mgmt="mixed-psk"
        [ -z "$whc_pswd" ] && whc_mgmt="none" || whc_pswd="$(base64_dec $whc_pswd)"
        [ -z "$ssid_game" ] && ssid_game="!@Mi-son_5G_Game" || ssid_game="$(base64_dec $ssid_game)"
		[ -z "$mgmt_game" ] && mgmt_game="mixed-psk"
        [ -z "$pswd_game" ] && mgmt_game="none" || pswd_game="$(base64_dec $pswd_game)"
		...
		[ -z "$bh_macnum_2g" -o "$bh_macnum_2g" -eq 0 ] && bh_maclist_2g=""
		[ -z "$bh_macnum_5g" -o "$bh_macnum_5g" -eq 0 ] && bh_maclist_5g=""
		MIMESH_LOGI " keys:<$whc_role>,<$bsd>,<$whc_ssid>,<$whc_pswd>,<$whc_mgmt>,<$ssid_game>,<$pswd_game>,<$mgmt_game>,<$bh_ssid>,<$bh_pswd>,<$bh_mgmt>"
		MIMESH_LOGI " keys:<$bh_macnum_2g>,<$bh_maclist_2g>,<$bh_macnum_5g>,<$bh_maclist_5g>"
