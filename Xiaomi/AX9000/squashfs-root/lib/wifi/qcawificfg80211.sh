...
update_global_daemon_coldboot_qdss_support_variables() {
	...
	*)
	...
	then
		g_daemon=`grep -i $board_name $fw_ini_file | grep "enable_daemon_support" | awk -F '=' '{print $2}' | grep -c 1`
		g_cold_boot_support=`grep -i $board_name $fw_ini_file | grep "enable_cold_boot_support" | awk -F '=' '{print $2}' | grep -c 1`
		g_qdss_tracing=`grep -i $board_name $fw_ini_file | grep "enable_qdss_tracing" | awk -F '=' '{print $2}' | grep -c 1`
...
start_recovery_daemon() {
	...
	pgrep acfg_tool > /dev/null && killall acfg_tool
	# miwifi disable output to console on default
	# /usr/sbin/acfg_tool -e -s > /dev/console &
	/usr/sbin/acfg_tool -e -s > /dev/null &
}
...
enable_qdss_tracing() {
	...
	do
		qdss_tracing=`grep -i $board_name $fw_ini_file | grep "enable_qdss_tracing" | grep -i $slot | awk -F'=' '{print $2}' | grep -c 1`
		[ $qdss_tracing = 1 ] && {
			echo "****** Starting QDSS for $slot ********" > /dev/console
			cnsscli -i $slot --qdss_start > /dev/null
		}
	done

	#For Integrated Radio
	qdss_tracing=`grep -i $board_name $fw_ini_file | grep "enable_qdss_tracing" | grep -v pci | awk -F'=' '{print $2}' | cut -c 1`
	[ -z $qdss_tracing ] && {
...
start_cnssdaemon() {
	...
	#For Integrated Radio
	daemon=`grep -i $board_name $fw_ini_file | grep "enable_daemon_support" | grep -v pci | awk -F'=' '{print $2}' | grep -c 1`
	if [ $daemon != 0 ]
	then
		args=" -i integrated"
	fi
	...
	for slot in pci0 pci1
	do
		daemon=`grep -i $board_name $fw_ini_file | grep "enable_daemon_support" | grep -i $slot | awk -F'=' '{print $2}' | grep -c 1`
		if [ $daemon != 0 ]
		then
			args="$args -i $slot"
		fi
...
update_daemon_cold_boot_support_to_plat_priv() {
	...
	for slot in pci0 pci1
	do
		daemon_support=`grep -i $board_name $fw_ini_file | grep "enable_daemon_support" | grep -i $slot | awk -F'=' '{print $2}' | grep -c 1`
		cold_boot_support=`grep -i $board_name $fw_ini_file | grep "enable_cold_boot_support" | grep -i $slot | awk -F'=' '{print $2}' | grep -c 1`
		[ $daemon_support = 1 ] && {
	...
	#For Integrated Radio
	daemon_support=`grep -i $board_name $fw_ini_file | grep "enable_daemon_support" | grep -v pci | awk -F'=' '{print $2}' | cut -c 1`
	[ -z $daemon_support ] && {
		daemon_support=`grep -i $daemon_features_default_string $fw_ini_file | grep "enable_daemon_support" | awk -F'=' '{print $2}' | grep -c 1`
	}

	cold_boot_support=`grep -i $board_name $fw_ini_file | grep "enable_cold_boot_support" | grep -v pci | awk -F'=' '{print $2}' | cut -c 1`
	[ -z $cold_boot_support ] && {
...
is_son_enabled()
{
	...
	if [ $is_son_hyd_enabled = "1" ] || [ $is_son_lbd_enabled = "1" ]
	then
		son_is_enabled=1
		echo $son_is_enabled
	else
...
load_qcawificfg80211() {
	...
	[ "$debugtxbf_ulofdma" = "Y" ] && echo_cmd Y /sys/module/cnss2/parameters/qca6290_support
	[ "$debugtxbf_ulofdma" = "N" ] && echo_cmd N /sys/module/cnss2/parameters/qca6290_support
	...
	*)
	...
	config_get bcast_twt_enable qcawifi bcast_twt_enable
	[ -n "$bcast_twt_enable" ] && update_ini_file bcast_twt_enable "$bcast_twt_enable"
...
unload_qcawificfg80211() {
	...
	pgrep iface-mgr > /dev/null  && killall iface-mgr
	pgrep qldtool > /dev/null && killall qldtool

	eval "type lowi_teardown" >/dev/null 2>&1 && lowi_teardown
	sleep 3
	pgrep cfr_test_app > /dev/null && killall cfr_test_app
	if [ "$recover" != "1" ]; then
		lock /var/run/wifilock
	fi
...
enable_qcawificfg80211() {
	...
	config_get dbglog_disable "$device" dbglog_disable
	[ -n "$dbglog_disable" ] && "$device_if" "$phy" dbglog_disable "${dbglog_disable}"
...
enable_vifs_qcawificfg80211() {
	...
	for vif in $vifs; do
		...
		if [ "$pmode" = "specialvap" ] || [ "$pmode" = "smart_monitor" ] || [ "$pmode" = "monitor" ]|| [ "$pmode" = "lite_monitor" ] || [ "$pmode" = "lp_iot_mode" ]; then
			...
			"$device_if" "$ifname" channel "$channel" "$band"
		...
		[ "$miwifi_mesh" -eq 1 ] && {
			local obdevice=$(uci -q get misc.wireless.if_5G)
			local other_bh_band_bssid=$(uci -q get wireless.$obdevice.macaddr)
			iwpriv $ifname miwifi_mesh_mac $other_bh_band_bssid
		}
		[ "$miwifi_mesh" -eq 4 ] && {
			local obdevice=$(uci -q get misc.wireless.if_game)
			local other_bh_band_bssid=$(uci -q get wireless.$obdevice.macaddr)
			iwpriv $ifname miwifi_mesh_mac $other_bh_band_bssid
		}
			...
			mixed*|wep*|psk*)
				...
				then
					...
					then
                        ...
                        # ignore the resule of "list" sae_password, xiaomi uses "option" sae_password instead.
...
#TODO
		...
		[ -n "$vht_mcs_10_11" ] && "$device_if" "$ifname" vht_mcs_10_11_nq2q_peer_supp "${intop}"
...
#TODO
		...
		if [ $ifname == "wl33" ] && [ $bindstatus == 0 -o $userswitch == 0 ];then
			hostapd_cli -i wl33 -p /var/run/hostapd-wifi1 disable
		fi

		local netmode=$(uci -q get xiaoqiang.common.NETMODE)
		if [ -n "$netmode" ] && [ "$netmode" = "whc_re" ]; then
			local bh_caps=$(uci -q get misc.backhauls.backhaul_caps)
			[ -z "$bh_caps" ] && bh_caps="game"
			local bh_tag=$(uci -q get misc.backhauls.backhaul)
			for bhdev in $bh_caps; do
				local backhaul_5g_ap_iface=$(uci -q get misc.backhauls.backhaul_${bhdev}_ap_iface)
		...
		#local ifname_5G=$(uci -q get misc.wireless.ifname_${bhdev})
	...
	#local netmode=$(uci -q get xiaoqiang.common.NETMODE)
	#if [ -n "$netmode" ] && [ "whc_re" = "$netmode" ]; then
	#	local bhdev="game"
	#	local backhaul_5g_sta_iface=$(uci -q get misc.backhauls.backhaul_${bhdev}_sta_iface)
	#	if [ $ifname = $backhaul_5g_sta_iface ]; then
	...
	case "${action}" in
		disable)
			...
			if [ -f /etc/init.d/lbd ]; then
				pgrep lbd > /dev/null && /etc/init.d/lbd stop
			fi
			if [ -f /etc/init.d/hyd ]; then
				pgrep hyd > /dev/null && /etc/init.d/hyd stop
			fi
			if [ -f /etc/init.d/ssid_steering ]; then
				pgrep ssidsteering > /dev/null && /etc/init.d/ssid_steering stop
			fi
			if [ -f /etc/init.d/mcsd ]; then
				pgrep mcsd > /dev/null && /etc/init.d/mcsd stop
			fi
			if [ -f /etc/init.d/wsplcd ]; then
				pgrep wsplcd > /dev/null && /etc/init.d/wsplcd stop
			fi
...
ftm_qcawificfg80211() {
	...
	echo_cmd -n "/ini" /sys/module/firmware_class/parameters/path
	case "$board_name" in
	ap-hk*|ap-dk*|ap-ac*|ap-cp*|ap-oa*|ap-mp*)
...
detect_qcawificfg80211() {
	...
	for dev in $(ls -d wifi* 2>&-); do
		...
		if [ $devidx = 0 ]; then
			...
			mode=ap
			network=lan
			ssid=`nvram get wl2_ssid`
			if [ "x$ssid" = "x" ]; then
				ssid=$(nvram get wl0_ssid)_Game
			fi
			ssid=${ssid}
		else
			htmode=HT20
...
EOF
	...
	if [ "$is_ftm" = "1" ] && [ "$devidx" = "1" -o "$devidx" = "3" ]; then
		cat <<EOF
	option bw '40'
EOF
	fi
		...
		*)
		...
		if [ ! -f /sys/firmware/devicetree/base/MP_256 ]; then
			if [ $num_chains -le 2 ]; then
				...
				update_ini_for_monitor_buf_ring QC5018_i.ini
			fi
		fi
