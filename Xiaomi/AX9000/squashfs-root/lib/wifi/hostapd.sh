...
hostapd_set_extra_cred() {
	...
	#wps_build_mac_addr
	# miwifi for waring when exec wifi
...
hostapd_set_bss_options() {
	...
	[ -n "$map" -a "$map" -gt 0 ] && append "$var" "multi_ap_profile=$map" "$N"
	...
	[ ! -z "${wps_sae}" -a "${wps_sae}" -gt 0 ] && [ ! -z "$wps_pbc" -a $wps_pbc -eq 1 ] && append "$var" "wps_cred_add_sae=1" "$N"
...
hostapd_setup_vif() {
	...
	[ "$channel" = "auto" -o "$channel" = "AUTO" ] && channel=0
