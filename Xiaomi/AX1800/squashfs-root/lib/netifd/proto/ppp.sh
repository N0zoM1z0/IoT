...
ppp_generic_setup() {
	...
	json_get_vars ipv6 ip6table demand keepalive keepalive_adaptive username password pppd_options pppname unnumbered persist maxfail peerdns
...
proto_pppoe_setup() {
	...
	local specdial
	local special='/tmp/state/pppoe_special'
	[ -f "$special" ] && specdial=$(cat "$special")

	local last_state
	local f_last_state='/tmp/state/pppoe_last_error'
	[ -f "$f_last_state" ] && last_state=$(cat "$f_last_state")
	[ "$last_state" = "11" -o "$last_state" = "19" ] && holdoff=30 || holdoff=2
...
proto_pppoe_teardown() {
        local last_state='/tmp/state/pppoe_last_error'
	local special='/tmp/state/pppoe_special'
	# save pppoe last error
        [ -f "$last_state" ] || mkdir -p '/tmp/state';
	echo "$ERROR" > "$last_state"
	case "$ERROR" in
	    11|19)
		local spec_val
		for i in 1 6 7; do
		    spec_val=$(cat "$special" 2>/dev/null)
		    if [ "$spec_val" = "$i" ]; then
			echo -n > "$special"
			continue
		    fi
