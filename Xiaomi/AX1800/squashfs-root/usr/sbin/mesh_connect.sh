...
init_cap_mode() {
	...
	local device_5g=$(uci -q get misc.wireless.if_5G)
	uci set wireless.${device_5g}.AutoChannelSkipList="52;56;60;64;100;104;108;112;116;120;124;128;132;136;140"
	uci commit wireless
}
...
do_cap_init_bsd() {
	...
	if [ "whc_cap" = "$mode" -o "$mode" = "lanapmode" -a "$cap_mode" = "ap" ]; then
		[ "$exist_5g" -eq 0 ] && {
			#iwpriv $ifname_5g set ACLAddEntry=$3
			#iwpriv $ifname_5g set ACLAddEntry=$5
			#iwpriv $ifname_5g set AccessPolicy=1
...
do_cap_init() {
	...
	local cap_mode=$(uci -q get xiaoqiang.common.CAP_MODE)
	local bh_ap_iface_5g=$(uci show wireless | grep -w "ifname='$bh_ifname_5g'" | awk -F"." '{print $2}')
	...
	if [ "whc_cap" = "$mode" -o "$mode" = "lanapmode" -a "$cap_mode" = "ap" ]; then
		[ "$exist_5g" -eq 0 ] && {
			#iwpriv $bh_ifname_5g set ACLAddEntry=$3
			#iwpriv $bh_ifname_5g set ACLAddEntry=$5
			#iwpriv $bh_ifname_5g set AccessPolicy=1
