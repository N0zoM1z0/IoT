...
for i in $list; do
	...
	log_exec "iwpriv wl$i show stainfo >> $LOG_TMP_FILE_PATH"
	log_exec "iwconfig wl$i >> $LOG_TMP_FILE_PATH"
	log_exec "iwpriv wl$i set ResetCounter=1"
	log_exec "sleep 1"
...
[ "$netmode" = "whc_cap" -o "$netmode" = "whc_re" ] && {
	[ "$netmode" = "whc_re" -a -n "$bh_sta_list" ] && {
		...
		for i in $bh_sta_list; do
			echo "========== @@@@ iwinfo apcl$i @@@@" >> $LOG_TMP_FILE_PATH
			echo "========== @@@@ iwinfo apcl$i @@@@" > /dev/console
			log_exec "iwinfo apcl$i info"
			log_exec "iwconfig apcl$i"
		done
	}
