...
topomon_wifi_if_down() {
	local sta_ifname=$1
	local sta_iface=$(uci show wireless | grep ".ifname='$sta_ifname'" | awk -F"." '{print $2}')
	local sta_disabled=$(uci -q get wireless.$sta_iface.disabled)

	if [ -n "$sta_ifname" ]; then
		log "topomon_wifi_if_down: sta_ifname $sta_ifname"

		if [ "$sta_disabled" == "0" ]; then
			log "topomon_wifi_if_up: uci set $sta_ifname disabled=1"
			uci set wireless.$sta_iface.disabled=1
			uci commit wireless

			if [ -f "/tmp/run/wifi.lock" ]; then
				for i in $(seq 1 15)
				do
					pid=$(cat /tmp/run/wifi.lock)
					res=`ps|awk '{print $1}'|grep $pid > /dev/null;echo $?`
					if [ "$res" == "0" ]; then
						sleep 2
						log "topomon_wifi_if_down: i = $i, delay ifconfig $sta_ifname down"
					else
						log "topomon_wifi_if_down: wifi shell not running, break..."
						break
					fi
				done
			fi

			log "topomon_wifi_if_down: ifconfig $sta_ifname down"
			ifconfig $sta_ifname down
			ubus call network.interface.lan remove_device "{\"name\":\"$sta_ifname\"}"
		fi
	fi
}
...
topomon_wifi_if_up() {
	...
	if [ -n "$sta_ifname" ]; then
		...
		log "topomon_wifi_if_up: sta_ifname $sta_ifname"
		if [ "$sta_disabled" == "1" ]; then
			log "topomon_wifi_if_up: uci set $sta_ifname disabled=0"
			uci set wireless.$sta_iface.disabled=0
			uci commit wireless

			log "topomon_wifi_if_up: ifconfig $sta_ifname up"
			ifconfig $sta_ifname up
			ubus call network.interface.lan add_device "{\"name\":\"$sta_ifname\"}"
			sleep 1
			iwpriv $sta_ifname set ApCliEnable=0
			iwpriv $sta_ifname set ApCliEnable=1
			iwpriv $sta_ifname set ApCliAutoConnect=3
			topomon_update_status "port_name" $sta_ifname
		fi
	fi
}
...
set_backhaul_ap_aplimit() {
	...
	if [ $hop_count = "0" ]; then
		iwpriv $bh_wlan_iface set mesh_aplimit=3
	elif [ $hop_count = "1" ]; then
		iwpriv $bh_wlan_iface set mesh_aplimit=2
	else
		iwpriv $bh_wlan_iface set mesh_aplimit=0
		iwpriv $bh_wlan_iface set DisConnectAllSta=1
	fi
}
...
topomon_set_connect_bssid() {
	...
	if [ -n "$sta_iface" -a -n $new_bssid ]; then
		if [ "$restart" -gt 0 ]; then
			...
			iwpriv $sta_iface set ApCliBssid=$new_bssid
		else
...
topomon_topo_update() {
	...
	if [ "$bh_type" = "wired" ]; then
		...
		iwpriv $bh_wlan_iface set mesh_ethmode=1
		iwpriv $bh_wlan_iface set mesh_capip=$int_ip
		iwpriv $bh_wlan_iface set mesh_ulrate=$uplink_rate
		iwpriv $bh_wlan_iface set mesh_hop=$hop_count
		backhaul_type=1
		arping -f -c 2 -I br-lan $(int2ip $int_ip)
		ubus call xq_info_sync_mqtt topo_changed
	elif [ "$bh_type" = "wireless" ]; then
		iwpriv $bh_wlan_iface set mesh_ethmode=0
		backhaul_type=2
		topomon_update_status "bh_type" $bh_type
		bh_sta_is_running=$(ifconfig $bh_sta_iface | grep -wc "RUNNING")
		if [ $bh_sta_is_running = "1" ]; then
			int_ip=$(iwpriv $bh_wlan_iface get mesh_capip | awk -F":" '{print $2}')
			uplink_rate=$(iwpriv $bh_wlan_iface get mesh_ulrate | awk -F":" '{print $2}')
			hop_count=$(iwpriv $bh_wlan_iface get mesh_hop | awk -F":" '{print $2}')
			uplink_mac=$(mtknetlink_cli $bh_wlan_iface mesh_ulmac | awk -F " " '{print $2}')
			topomon_update_status "eth_link_rate" $uplink_rate
			topomon_update_status "hop_count" $hop_count
			ubus call xq_info_sync_mqtt topo_changed
		else
			...
			hop_count=255
			topomon_update_status "hop_count" $hop_count
			iwpriv $bh_sta_iface set ApCliAutoConnect=3
		fi
	else
		hop_count=255
		iwpriv $bh_wlan_iface set mesh_hop=$hop_count
...
topomon_check_best_bssid() {
	...
	local best_bssid=$(mtknetlink_cli $sta_iface mesh_bssid | awk -F " " '{print $2}')
...
topomon_init() {
	...
	if [ "$bh_type" = "wired" ]; then
		...
		topomon_wifi_if_up $bh_sta_iface
...
topomon_update_cap_wifi_param() {
	...
	iwpriv $bh_wlan_iface set mesh_capip=$int_ip
	iwpriv $bh_wlan_iface set mesh_hop=0
...
topomon_wireless_update() {
	...
	local bh_wlan_iface=$(uci -q get misc.backhauls.backhaul_5g_ap_iface)
	local int_ip=$(iwpriv $bh_wlan_iface get mesh_capip | awk -F":" '{print $2}')
	local str_ip=$(int2ip $int_ip)
	local current_ip=$(topomon_current_status "cap_ip")
	local hop_changed=0
	...
	local uplink_rate=$(iwpriv $bh_wlan_iface get mesh_ulrate | awk -F":" '{print $2}')
	...
	local hop_count=$(iwpriv $bh_wlan_iface get mesh_hop | awk -F":" '{print $2}')
	...
	local current_uplink_mac=$(topomon_current_status "uplink_mac")
	local uplink_mac=$(mtknetlink_cli $bh_wlan_iface mesh_ulmac | awk -F " " '{print $2}')
...
topomon_cac_status_check() {
	local ifname=$1
	local status=$(iwpriv $ifname get DfsRDMode | cut -d ':' -f 2)
...
topomon_update_re_wifi_param() {
	...
	if [ "$bh_type" = "wired" ]; then
		iwpriv $backhaul_5g_ap_iface set mesh_ethmode=1
		...
		[ -n $str_ip ] && {
			...
			iwpriv $backhaul_5g_ap_iface set mesh_capip=$bin_ip
		}
		[ -n $hop_count ] && iwpriv $backhaul_5g_ap_iface set mesh_hop=$hop_count
		[ -n $ulrate ] && iwpriv $backhaul_5g_ap_iface set mesh_ulrate=$ulrate
	else
		iwpriv $backhaul_5g_ap_iface mesh_ethmode 0
	fi
...
trigger_dhcp_new_ip(){
	...
	# walk around for apclii0 not in br-lan
	local curr_bh_type=$(topomon_current_status "bh_type")
	[ "$curr_bh_type" = "wireless" ] && {
		local bh_sta_ifname=$(uci -q get misc.backhauls.backhaul_5g_sta_iface)
		local bh_sta_in_br=$(brctl show | grep -wc $bh_sta_ifname)
		[ "$bh_sta_in_br" = "0" ] && {
			ubus call network.interface.lan add_device "{\"name\":\"$bh_sta_ifname\"}"
		}
	}

	if [ -f "$pid_file" ]; then
		#trigger udhcpc to renew/rebound DHCP-IP
		log "trigger_dhcp_new_ip..."
