...
scan_channel()
{
	...
	local cchannel_line=$(expr $cchannel + 15)
	local cscore=$(awk NR==${cchannel_line} ${ACSREPORT_FILE} | awk -F ")" '{print $2}' | awk -F " " '{print $5}')
	...
	do
		...
		score_index=$(awk NR==${line} ${ACSREPORT_FILE} | awk -F ")" '{print $2}' | awk -F " " '{print $5}')
