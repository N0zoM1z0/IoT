...
request_dinfo() {
	...
	mjac_i2c=$(uci -q get  miio_ot.ot.mjac_i2c)
	mjac_gpio=$(uci -q get  miio_ot.ot.mjac_gpio)
	...
	[ -n "$mjac_i2c" ] && json_add_string "mjac_i2c" "$mjac_i2c"
	[ -n "$mjac_gpio" ] && json_add_string "mjac_gpio" "/sys/class/gpio/gpio$mjac_gpio/value"
...
internal_info() {
	...
	# get wan_dev in default route
	wan_dev=$(ip route show default | grep -o "dev [^ ]*" | head -n 1 | awk '{print $2}' 2>/dev/null)

	if [ -n "$wan_dev" ]; then
		# get wan ip and mask of wan_dev in ifconfig
		wan_ip=$(ifconfig "$wan_dev" | grep -o "inet addr:[^ ]*" | head -n 1  | awk -F: '{print $2}' 2>/dev/null)
		wan_mask=$(ifconfig "$wan_dev" | grep -o "Mask:[^ ]*" | head -n 1  | awk -F: '{print $2}' 2>/dev/null)

		# get wan gateway
		wan_gw=$(ip route show default | grep -o "via [^ ]*" | head -n 1 | awk '{print $2}' 2>/dev/null)
		if [ -z "$wan_gw" ]; then
			# no gateway, means PtP link, get the first route in wan_dev
			wan_gw=$(ip route show dev "$wan_dev" | grep -v default | head -n 1 | awk '{print $1}' 2>/dev/null)
		fi
	fi
	...
	json_add_string "localIp" "$wan_ip"
	json_add_string "mask" "$wan_mask"
	json_add_string "gw" "$wan_gw"
	json_close_object
