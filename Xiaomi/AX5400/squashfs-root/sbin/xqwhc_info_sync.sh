...
wifi_parse()
{
    ...
    uplink_backhaul_2g="`uci -q show misc.backhauls.backhaul_2g_ap_iface|awk -F "'" '{print $2}'`"
...
system_parse()
{
    ...
    [ "$led_blue" != "$led_blue_cur" ] && {
	    ...
	    ubus call wan_check blink_off
            ...
            xqled sys_ok
            [ -f /usr/sbin/wan_check.sh ] && /usr/sbin/wan_check.sh reset
            [ -f /etc/init.d/scan ] && /etc/init.d/scan led_reset
        fi
    ...
    [ "$ethled" != "$ethled_cur" ] && {
        ...
        uci set xiaoqiang.common.ETHLED="$ethled"
        uci commit xiaoqiang

        if [ $ethled -eq 0 ];then
            [ -f /usr/sbin/ethled_ctl ] && /usr/sbin/ethled_ctl ethled_off
        else
            [ -f /usr/sbin/ethled_ctl ] && /usr/sbin/ethled_ctl ethled_on
        fi
    }

    local xled="`cat $cfgf | grep -w "xled" | awk -F ":=" '{print $2}'`"
    [ -n "$xled" ] && {
        local xled_cur="`uci -q get miled.settings.status`"
        [ -z "$xled_cur" ] && xled_cur=0
        [ "$xled" != "$xled_cur" ] && {
            sys_changed=0
            WHC_LOGI " xq_whc_sync, x led change $xled_cur -> $xled"
            uci set miled.setting.status="$xled"
            uci commit miled

            if [ $xled -eq 0 ];then
                [ -f /usr/sbin/xled_ctl ] && /usr/sbin/xled_ctl xled_off
            else
                 [ -f /usr/sbin/xled_ctl ] && /usr/sbin/xled_ctl xled_on
            fi
        }
    }
