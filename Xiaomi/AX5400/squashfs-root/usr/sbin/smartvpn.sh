...
smartvpn_vpn_route_add()
{
    ...
    ip rule del from $(fix_subnet $subnet) table vpn &>/dev/null
    ip rule add from $(fix_subnet $subnet) table vpn prio $smartvpn_prio &>/dev/null 
}
...
smartvpn_wandns2vpn_change()
{
    ...
    network_get_dnsserver wan_dnsservers wan
...
smartvpn_vpn_mark_redirect_open()
{
    ...
    ip rule add fwmark $ipset_mark table vpn prio $smartvpn_prio &>/dev/null
    ...
    if [ "$smartvpn_cfg_devicedisabled" = "0" -a -n "$smartvpn_cfg_devicemac" ]; then
        ...
        do
            ...
            iptables -t mangle -A $smartvpn_device_table  -m mac --mac-source $mac -j RETURN &>/dev/null
        done
