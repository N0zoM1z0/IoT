...
readonly PPPOE_WAN2_ITF="pppoe-wan2"
wanip=""
...
get_wan2_ip_info() {
	...
	local wanitf=$(uci -q get network.wan2.ifname)
	[ "$proto" = "pppoe" ] && wanitf=$PPPOE_WAN2_ITF
	wan2ip=$(ifconfig $wanitf | grep "inet addr" | cut -d ':' -f 2 | cut -d ' ' -f 1)
	wan2netmask=$(ifconfig $wanitf | grep "Mask" | cut -d ':' -f 4)
}
...
ip_conflict_detection() {
	...
	local wan2_enable=$(uci -q get dualwan.common.enable)
	[ "$wan2_enable" = "1" ] && {
		get_wan2_ip_info
		[ -z "$wan2ip" -o -z "$wan2netmask" ] && echo "0.0.0.0" && return
		local wan2ip_int=$(ip_aton $wan2ip)
		local wan2netmask_int=$(ip_aton $wan2netmask)
	}
	...
	[ "$res" = "1" ] && {
		...
		res=$(is_same_subnet $new_int $wan2ip_int $lannetmask_int $wan2netmask_int)
		[ "$res" = "1" ] && {
			newip_int=$(get_new_ip $lanip_int $lannetmask $wannetmask 7)
			newip=$(ip_ntoa $newip_int)
		}
		echo $newip && return
	}

	# check wan2, then wan1
	res=$(is_same_subnet $lanip_int $wan2ip_int $lannetmask_int $wan2netmask_int)
	[ "$res" = "1" ] && {
		newip_int=$(get_new_ip $lanip_int $lannetmask $wan2netmask 3)
		newip=$(ip_ntoa $newip_int)
		res=$(is_same_subnet $new_int $wanip_int $lannetmask_int $wannetmask_int)
		[ "$res" = "1" ] && {
			newip_int=$(get_new_ip $lanip_int $lannetmask $wan2netmask 7)
			newip=$(ip_ntoa $newip_int)
		}
		echo $newip && return
	}
...
miot_conflict_resolution() {
	...
	[ "$res" = "1" ] && {
		...
		local configured=$(uci -q get xiaoqiang.common.INITTED)
		[ "$configured" = "YES" ] && ubus call network reload 2>/dev/null
		/etc/init.d/dnsmasq stop 2>/dev/null
		/etc/init.d/dnsmasq start 2>/dev/null
		/etc/init.d/trafficd restart 2>/dev/null
		/etc/init.d/lag reset 2>/dev/null
